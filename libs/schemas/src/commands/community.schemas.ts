...
export const CreateCommunity = {
  input: z
    ...
    .refine(
      (input) =>
        (input.base === ChainBase.Ethereum && input.eth_chain_id) || true,
      'Ethereum chain ID not provided',
    )
    .refine(
      (input) =>
        (input.base === ChainBase.CosmosSDK && input.cosmos_chain_id) || true,
      'Cosmos chain ID not provided',
    ),
  output: Community,
};
