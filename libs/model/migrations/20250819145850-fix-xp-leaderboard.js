  ...
  async up(queryInterface, Sequelize) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      for (const { id } of questIds) {
        const viewName = `quest_${id}_xp_leaderboard`;
        ...
        await queryInterface.sequelize.query(
          ...
          CREATE MATERIALIZED VIEW "${viewName}" AS
            WITH user_xp_combined AS (
                ...
                WHERE l.user_id IS NOT NULL AND q.id = ${id}
                ...
                FROM "XpLogs" l
                         JOIN "QuestActionMetas" m ON l.action_meta_id = m.id
                         JOIN "Quests" q ON m.quest_id = q.id
                WHERE l.creator_user_id IS NOT NULL AND q.id = ${id}
            ),
                 aggregated_xp AS (
                     SELECT
                         user_id,
            ...
            SELECT
                a.user_id,
                (a.total_user_xp + a.total_creator_xp) as xp_points,
                u.tier,
