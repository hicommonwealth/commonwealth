...
export function ClaimTokenCliff(): Command<typeof schemas.ClaimToken> {
  return {
    ...schemas.ClaimToken,
    auth: [],
    secure: true,
    body: async ({ payload, actor }) => {
      ...
      const [claim] = await models.sequelize.query<{
        ...
        `
          SELECT
            ...
            C.magna_cliff_claimed_at,
            C.magna_cliff_claim_data,
            C.magna_cliff_claim_tx_hash
          FROM
            "ClaimAddresses" C
          WHERE
            C.user_id = :user_id
      ...
      if (!claim.magna_claimed_at)
        throw new InvalidState('Must claim before cliff!');

      if (!claim.magna_cliff_claimed_at || !claim.magna_cliff_claim_data) {
        ...
        if (
          ...
          SET 
            magna_cliff_claimed_at = NOW(),
            magna_cliff_claim_data = :claim_data
          WHERE
            user_id = :user_id`,
            {
      ...
      const claim_data = JSON.parse(claim.magna_cliff_claim_data) as MagnaClaim;
      return {
        magna_allocation_id: claim.magna_allocation_id,
        from: claim_data.from as `0x${string}`,
        to: claim_data.to as `0x${string}`,
        data: claim_data.data,
        transaction_hash: claim.magna_cliff_claim_tx_hash as `0x${string}`,
      };
    },
  };
}
