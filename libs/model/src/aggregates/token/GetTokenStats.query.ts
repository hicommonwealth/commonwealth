  ...
  return {
    ...
    async body({ payload }) {
      ...
      const sql = `SELECT
        ...
        COALESCE(SUM(CASE WHEN timestamp >= EXTRACT(EPOCH FROM CURRENT_TIMESTAMP - INTERVAL '24 hours') THEN price * community_token_amount ELSE 0 END),0) AS volume_24h
        FROM "LaunchpadTrades" WHERE token_address = :token_address`;
      const [row] = await models.sequelize.query<{ holder_count: number; volume_24h: string }>(sql, {
