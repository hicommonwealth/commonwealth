...
export function createTradeHandler(isThreadToken: boolean) {
  return async ({
    payload,
  }: {
    payload: z.infer<typeof schemas.CreateLaunchpadTrade.input>;
  }) => {
    const { eth_chain_id, transaction_hash } = payload;

    const Model = isThreadToken ? models.LaunchpadTrade : models.LaunchpadTrade;

    const existingTrade = await Model.findOne({
      where: {
        eth_chain_id,
        transaction_hash,
      },
    ...
    const parsedLogs = parseEventLogs({
      abi: isThreadToken ? TokenBondingCurveAbi : LPBondingCurveAbi,
      eventName: 'Trade',
      logs: receipt.logs,
    });
