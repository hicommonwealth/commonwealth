...
export const notifyReferrerCommunityJoined: EventHandler<
  'CommunityJoined',
  z.ZodBoolean
> = async (event: EventContext<'CommunityJoined'>) => {
  const { user_id, referrer_address } = event.payload;
  ...
  const community = await models.Community.findOne({
    where: { id: event.payload.community_id },
