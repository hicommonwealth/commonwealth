...
// These names do NOT match 1 to 1 with an on-chain name. Rather these names are descriptive and specific to out app.
export type ChainEventNames =
  | 'DeployedNamespace'
  | 'NewContest'
  | 'NewRecurringContestStarted'
...
export const eventInputs: Record<
  ChainEventNames,
  Array<{
    name: string;
    type: string;
...
// function mapEventSignatureToEventInputs(eventSignature: ): typeof eventInputs {}

export const parseEvmEvent = (
  chainEventName: ChainEventNames,
  contractAddress: string,
  eventSignature: string,
  data: string,
  topics: string | Array<string>,
) => {
  return decodeLog(eventInputs[chainEventName], data, topics) as any;
};
