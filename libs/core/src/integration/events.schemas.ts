...
type ParseSignature<S extends string> =
  S extends `${infer Type} ${infer Name}, ${infer Rest}`
    ? { [K in Name]: ParseType<Type> } & ParseSignature<Rest>
    : S extends `${infer Type} ${infer Name}`
    ? { [K in Name]: ParseType<Type> }
    : any;

type EvmMapper<Input extends string, Output extends ZodSchema> = {
  signature: Input;
  condition: (input: ParseSignature<Input>) => boolean;
  mapToSchema: (input: ParseSignature<Input>) => z.infer<Output>;
};

const ChainEventSigs = {
  NewContestSig: 'address contest, address namespace, uint256 interval, bool oneOff' as const,
  NewRecurringContestStartedSig: 'uint256 indexed contestId, uint256 startTime, uint256 endTime' as const,
  NewSingleContestStartedSig: 'uint256 startTime, uint256 endTime' as const,
  ContentAddedSig: 'uint256 indexed contentId, address indexed creator, string url' as const,
  VoterVoted: 'address indexed voter, uint256 indexed contentId, uint256 votingPower' as const,
  PrizeShareUpdated: 'uint256 newPrizeShare' as const
}

const RecurringContestManagerDeployedMapper: EvmMapper<
  typeof ChainEventSigs.NewContestSig,
  typeof RecurringContestManagerDeployed
> = {
  signature: ChainEventSigs.NewContestSig,
  condition: (input) => !input.oneOff,
  mapToSchema: (input) => ({
    created_at: new Date(),
    namespace: input.namespace,
    contest_address: input.contest,
    interval: ethers.BigNumber.from(input.interval).toNumber(),
  }),
};
...
// parseEthersResult converts the ethers params into key-value pairs based on signature
...
const mapEvmToSchema = (
  chainEventName: ChainEventName,
  params: ethers.utils.Result,
) => {
  for (const [mapperEventName, mappers] of Object.entries(EvmMappers)) {
    for (const mapper of mappers) {
      const input = parseEthersResult(mapper.signature, params);
      if (chainEventName === mapperEventName && mapper.condition(input)) {
        return mapper.mapToSchema(input);
      }
    }
  }
