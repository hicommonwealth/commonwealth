...
3. Run against the built site in the packages/commonwealth folder with:
    ...
    ```bash
    NO_TOKEN_BALANCE_CACHE=true NO_PRERENDER=true NO_CLIENT_SERVER=true MIXPANEL_DEV_TOKEN=foo MIXPANEL_PROD_TOKEN=bar NODE_ENV=production npx ts-node -r tsconfig-paths/register -P tsconfig.server.json -T server.ts
    ```
