...
function truncateAddress(
  address: string,
  opts?: { numStartChars?: number; numEndChars?: number },
) {
  const opts_ = opts || {};

  // don't handle 0 as undefined
  const numStartChars =
    opts_.numStartChars === 0 ? 0 : opts_.numStartChars || 8;
...
export function DidPopover({
  ...
  const truncatedAddress = didAddress
    ? truncateAddress(didAddress, { numStartChars, numEndChars })
    : '';
    ...
    <Popover.Root>
      <Popover.Trigger>
        <Box display="inline-block">
          {truncateBelow ? (
            <>
              <Box display={{ [truncateBelow]: 'none' }}>
                {truncatedAddress}
              </Box>
              <Box
                display={{ initial: 'none', [truncateBelow]: 'inline-block' }}
              >
                {didAddress}
              </Box>
            </>
