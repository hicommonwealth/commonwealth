...
function SessionField({
  ...
  return (
    <span className="text-gray-400">
      {' '}
      {session && formatDistanceCustom(session.context.timestamp)} ago
    </span>
  );
}
  ...
  return (
    <Flex direction="column" gap="2" pt="4">
            ...
            ([cid, signature, message]: [
              ...
              return (
                <Table.Row key={cid}>
                  ...
                  <Table.Cell>
                    {args.length > 50 ? <ArgsPopout data={args} /> : args}
                  </Table.Cell>
                  <Table.Cell>
                    {formatDistanceCustom(message.payload.context.timestamp)}{' '}
                    ago
                  </Table.Cell>
                  <Table.Cell>
                    <DidPopover
                      did={signature.publicKey || ''}
                      truncateBelow="xl"
                    />
      ...
      <Flex direction="row" gap="2">
        ...
        <PaginationButton
          text="Previous"
          enabled={currentCursor !== null}
          onClick={popCursor}
        />
        <PaginationButton
          text="Next"
          enabled={hasMore}
          onClick={() => pushCursor(actions[entriesPerPage][0])}
        />
      </Flex>
    </Flex>
  );
