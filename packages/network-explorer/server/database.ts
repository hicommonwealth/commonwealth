...
export async function createDatabase(client: Client) {
  ...
  return {
    ...
    selectCountsAll: async (): Promise<QueryResult<Count>> => {
      return await client.query<Count>(
        'SELECT topic, type, COUNT(*)::int FROM messages GROUP BY topic, type;',
      );
    },

    selectCountsForTypeTotal: async (
      type: string,
    ): Promise<QueryResult<{ count: number }>> => {
      return await client.query<{ count: number }>(
        'SELECT COUNT(*)::int AS count FROM messages WHERE type = $1',
        [type],
      );
    },
    ...
    selectAddressCount: async (
      topic: string,
    ): Promise<QueryResult<AddressCount>> => {
      return await client.query<AddressCount>(
        'SELECT COUNT(*)::int AS count FROM addresses WHERE topic = $1',
        [topic],
      );
    },

    selectAddressCountsAll: async (): Promise<
      QueryResult<{ topic: string; count: string }>
    > => {
    ...
    selectAddressCountTotal: async (): Promise<QueryResult<AddressCount>> => {
      return await client.query<AddressCount>(
        'SELECT COUNT(DISTINCT address)::int AS count FROM addresses',
      );
    },
