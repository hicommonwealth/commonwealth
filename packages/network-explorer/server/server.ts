...
const topics = process.env.TOPIC ? [process.env.TOPIC] : ['common.xyz'];
...
const client = new pg.Client({
  connectionString:
    ...
    'postgresql://test@localhost:5432/network-explorer',
});
...
for (const topic of topics) {
  ...
  canvasApp.addEventListener('message', async (event) => {
    ...
    if (message.message.payload.type === 'action') {
      ...
      await queries.addAddress(
        message.message.topic,
        message.message.payload.did,
      );
    }
  });
...
expressApp.get('/index_api/messages', ipld(), async (req, res) => {
  ...
  const messageIndexEntries = await queries.selectAllMessages(
    before,
    numMessagesToReturn,
  );
...
expressApp.get('/index_api/messages/:topic', ipld(), async (req, res) => {
  ...
  const messageIds = await queries.selectMessages(
    req.params.topic,
    before,
    type,
    numMessagesToReturn,
...
expressApp.get('/index_api/counts', async (req, res) => {
  ...
  for (const row of addressCountResult) {
    ...
    connectionCountsMap[row.topic] =
      canvasApps[row.topic]?.libp2p.getConnections().length;
...
expressApp.get('/index_api/counts/total', async (req, res) => {
  const actionCount = (await queries.selectCountsForTypeTotal('action'))
    .rows[0];
  const sessionCount = (await queries.selectCountsForTypeTotal('session'))
    .rows[0];
...
expressApp.get('/index_api/counts/:topic', async (req, res) => {
  ...
  const addressCountResult = (
    await queries.selectAddressCount(req.params.topic)
  ).rows[0];
  const result = {
    ...
    address_count: addressCountResult.count || 0,
    connection_count:
      canvasApps[req.params.topic]?.libp2p.getConnections().length || 0,
  };
  res.json(result);
});
...
expressApp.get('/index_api/latest_session/:topic', async (req, res) => {
  ...
  const message = (await canvasApp.getMessage(sessionId))[1];
