...
export class compoundGovernor implements IGovernor  {
    ...
    public async createArbitraryProposal(advanceDays?: string | number): Promise<string> {
        ...
        .send({from: "0xF977814e90dA44bFA03b6295A0616a897441aceC", gasLimit: 100000})
        // Delegate to proposer
        await compToken.methods.delegate(accounts[0]).send({from: accounts[2], gasLimit: 150000})
        // Create Proposal
        ...
        return String(provider.eth.abi.decodeParameter('uint256', String(proposalId['events']["ProposalCreated"]["raw"]["data"]).slice(0,66)))
    }
    ...
    public async cancelProposal(proposalId: string | number): Promise<void> {
        ...
        const contract = comp_gov(this.contractAddress, provider);
        const accounts = (await provider.eth.getAccounts());
        const cancel = await contract.methods.cancel(proposalId).send({from: accounts[0], gasLimit:150000});
        console.log(cancel)
    }
    ...
    public async castVote(
        ...
        accountIndex: number,
        forAgainst: boolean): Promise<void> {
        
        const provider = getProvider();
        const contract = comp_gov(this.contractAddress, provider);
        const accounts = (await provider.eth.getAccounts())[accountIndex]
        const vote = await contract.methods.castVote(proposalId, Number(forAgainst)).send({from: accounts, gasLimit: 1000000})
        console.log(vote)
    }
    ...
    public async queueProposal(proposalId: string | number, advanceTime?: boolean): Promise<void> {
        if(advanceTime){
        ...
        const queued = await contract.methods.queue(proposalId).send({from: accounts, gasLimit: 500000})
        console.log(queued)
    }

    public async executeProposal(proposalId: string | number, advanceTime?: boolean): Promise<void> {
        if(advanceTime){
        ...
        const contract = comp_gov(this.contractAddress, provider);
        const accounts = (await provider.eth.getAccounts())[0]
        const executed = await contract.methods.execute(proposalId).send({from: accounts, value: 0, gasLimit: 1000000})
        console.log(executed)
    }
    
    public async getVotes(accountIndex: number, numberOfVotes: string): Promise<void>{
        const provider = getProvider();
        const compToken = erc20(this.compToken, provider)
        const accounts = (await provider.eth.getAccounts())[accountIndex]
        const currBalance = provider.utils.toBN(await compToken.methods.balanceOf(accounts).call())
        const tokensNeeded = provider.utils.toBN(provider.utils.toWei(numberOfVotes));
        if(currBalance.lt(tokensNeeded)){
            try{
                ...
                await compToken.methods
                .transfer(accounts, tokensNeeded)
                .send({from: "0xfA9b5f7fDc8AB34AAf3099889475d47febF830D7", gasLimit: 100000})   
            }
        }
        try{
            await compToken.methods.delegate(accounts).send({from: accounts, gasLimit: 150000})
        }catch{
