...
import { dexGetTokens, erc20Approve, erc20BalanceReq, erc20Transfer } from '../types';
...
export const getBalance =
 async (req: Request, res: Response) => {
    ...
    let balance: string = await contract.methods.balanceOf(request.address).call()
    if(request.convert){
        //TODO: dynamic
...
export const transfer
 = async (req: Request, res: Response) => {
    ...
    if(request.from){
        ...
        .transferFrom(request.to, request.from, Web3.utils.toWei(request.amount))
...
export const getTokens =
 async (req: Request, res: Response) => {
    ...
    const provider = getProvider()
    const account = (await provider.eth.getAccounts())[request.accountIndex ?? 0];
    const contract = uniswapV2("0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D", provider)
