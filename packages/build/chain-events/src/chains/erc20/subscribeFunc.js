...
async function createApi(ethNetworkUrl, tokenAddresses, tokenNames, retryTimeMs = 10 * 1000) {
    const log = logging_1.factory.getLogger(logging_1.addPrefix(__filename, [interfaces_1.SupportedNetwork.ERC20]));
    for (let i = 0; i < 3; ++i) {
        try {
            const provider = await eth_1.createProvider(ethNetworkUrl, interfaces_1.SupportedNetwork.ERC20);
