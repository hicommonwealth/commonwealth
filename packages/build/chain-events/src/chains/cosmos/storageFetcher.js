...
class StorageFetcher extends interfaces_1.IStorageFetcher {
    ...
    constructor(_api, chain) {
        super(_api);
        this._api = _api;
        this.log = logging_1.factory.getLogger(logging_1.addPrefix(__filename, [interfaces_1.SupportedNetwork.Cosmos, chain]));
    }
    _currentBlock;
    ...
    async _proposalToEvents(proposal) {
        ...
        const submitEvent = {
            ...
            data: {
                ...
                totalDeposit: proposal.totalDeposit && types_1.coinToCoins(proposal.totalDeposit),
            },
        };
        ...
        if (proposal.status === gov_1.ProposalStatus.PROPOSAL_STATUS_DEPOSIT_PERIOD) {
            ...
            const depositEvents = deposits.map((d) => ({
                ...
                data: {
                    ...
                    amount: types_1.coinToCoins(d.amount),
                },
            }));
