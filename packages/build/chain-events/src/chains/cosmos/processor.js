...
class Processor extends interfaces_1.IEventProcessor {
     ...
     */
    async process(event) {
        const log = logging_1.factory.getLogger(logging_1.addPrefix(__filename, [interfaces_1.SupportedNetwork.Cosmos, this.chain]));
        const kind = type_parser_1.ParseType(event.message.typeUrl, this.chain);
        if (!kind)
            return [];
        try {
            const cwEvent = await enricher_1.Enrich(this._api, event.height, kind, event);
            return [cwEvent];
        }
        catch (e) {
