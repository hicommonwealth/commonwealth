...
async function createApi(ethNetworkUrl, factoryAddress, retryTimeMs = 10 * 1000, chain) {
    ...
    for (let i = 0; i < 3; ++i) {
        try {
            const provider = await eth_1.createProvider(ethNetworkUrl, interfaces_1.SupportedNetwork.Commonwealth, chain);
            ...
            for (let projectN = 1; projectN <= nProjects; projectN++) {
                ...
                const project = await subscriber_1.constructProjectApi(api.factory, projectAddress);
