...
exports.RedisCache = exports.redisRetryStrategy = void 0;
...
function redisRetryStrategy(retries) {
    if (retries > 5) {
...
class RedisCache {
    ...
    async init() {
        ...
        log.info(`Connecting to Redis at: ${config_1.REDIS_URL}`);
        const localRedis = config_1.REDIS_URL.includes('localhost') || config_1.REDIS_URL.includes('127.0.0.1');
        const vultrRedis = config_1.REDIS_URL.includes(config_1.VULTR_IP);
        if (!this.client) {
            const redisOptions = {};
            redisOptions['url'] = config_1.REDIS_URL;
            ...
            this.client = redis_1.createClient(redisOptions);
        }
        this.client.on('error', (err) => {
            if (err instanceof redis_1.ConnectionTimeoutError) {
                log.error(`RedisCache connection to ${config_1.REDIS_URL} timed out!`);
            }
            else if (err instanceof redis_1.ReconnectStrategyError) {
                log.error(`RedisCache max connection retries exceeded!`);
                if (!localRedis && !vultrRedis)
                    this.rollbar.critical('RedisCache max connection retries exceeded! RedisCache client shutting down!');
            }
            else if (err instanceof redis_1.SocketClosedUnexpectedlyError) {
     ...
     */
    async setKey(namespace, key, value) {
        if (!this.initialized) {
    ...
    async setKeys(namespace, data) {
        ...
        try {
            ...
            log.error(`An error occurred while setting the following data: ${data}`, e);
            return false;
        }
        return true;
    }
     ...
     */
    async getNamespaceKeys(namespace, maxResults) {
        if (!this.initialized) {
            log.error('Redis client is not initialized. Run RedisCache.init() first!');
            return false;
        }
