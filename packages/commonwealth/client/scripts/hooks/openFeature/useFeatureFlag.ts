...
import { useOpenFeatureClient } from './OpenFeatureProvider';
...
export function useBooleanFlagDetails(
  ...
  return useAttachHandlersAndResolve(
    flagKey,
    defaultValue,
    (client) => {
...
export function useObjectFlagValue<T extends JsonValue = JsonValue>(
  ...
  defaultValue: T,
  options?: ReactFlagEvaluationOptions,
): boolean | string | number | JsonObject | JsonValue[] {
...
function useAttachHandlersAndResolve<T extends FlagValue>(
  flagKey: string,
  defaultValue: T,
  resolver: (
    client: Client,
  ) => (flagKey: string, defaultValue: T) => EvaluationDetails<T>,
  options?: ReactFlagEvaluationOptions,
): EvaluationDetails<T> {
  const [, updateState] = useState<object | undefined>();
  const defaultedOptions: any = { ...DEFAULT_OPTIONS, ...options };
  const forceUpdate = () => {
    updateState({});
  };
