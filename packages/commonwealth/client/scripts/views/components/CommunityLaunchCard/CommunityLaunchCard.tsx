...
export const CommunityLaunchCard: React.FC<CommunityLaunchCardProps> = ({
  onLaunch,
}) => {
        ...
        messages: [
          {
            role: 'user',
            content:
              'Generate a short, catchy name for a new community. Keep it concise and memorable.',
          },
        ],
      });
        ...
        messages: [
          {
            role: 'user',
            content:
              "Generate a short, compelling description for a community called '" +
              name +
  ...
  const handleLaunch = useCallback(async () => {
    const ethereumAddress = user.addresses.find(
      (addr) => addr.community.base === ChainBase.Ethereum,
    );
    ...
    try {
      ...
      navigateToCommunity({
        navigate,
        path: '',
        chain: communityData.name.toLowerCase(),
      });
    } catch (err) {
  ...
  const handleImageUpload = useCallback(
    (uploadedImage: File, imageUrl: string) => {
    ...
    async (event: React.MouseEvent<HTMLButtonElement>) => {
      try {
          ...
          messages: [
            {
              role: 'user',
              content:
                "a compelling, funny, catchy community that people will buy. It should be a short tweet length, to the point, lowercase only. Don't include filler, just show the idea",
            },
          ],
        });
        ...
        if (generatedImageURL) {
          ...
          const file = new File([blob], 'community-image.png', {
            type: 'image/png',
          });
      ...
      <div className="bottom-row">
        ...
        <div className="right-buttons">
          ...
          <CWButton
            ...
            buttonWidth="narrow"
          >
            {isLoading ? 'Loading...' : isLoggedIn ? 'Launch' : 'Must Sign In'}
          </CWButton>
        </div>
      </div>
