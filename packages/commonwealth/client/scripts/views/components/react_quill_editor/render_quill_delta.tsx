...
export const renderQuillDelta = (
  ...
  console.log('convertedDelta', convertedDelta);
  ...
  // console.log('groups', groups[0].parents[0].attributes.['code-block']);
  ...
  ) : groups && groups[0].parents[0].attributes?.['code-block'] ? (
    <div>
      <pre>
        <code>{convertedDelta}</code>
      </pre>
    </div>
  ) : (
    consolidateOrderedLists(groups).map((group, index) => {
      const renderChild = (child, ii, attr) => {
        // handle images
