...
class EditNewProfile implements m.Component<EditNewProfileAttrs> {
  address: string;
  profile: Profile;
  profileUpdate: any;
  saved: boolean;
  ...
  updateProfile = async (vnode) => {
    const response: any = await $.post(`${app.serverUrl()}/updateProfile/v2`, {
      ...
      ...('email' in vnode.state.profileUpdate && {
        email: vnode.state.profileUpdate.email,
      }),
      ...('name' in vnode.state.profileUpdate && {
        name: vnode.state.profileUpdate.name,
      }),
      ...('bio' in vnode.state.profileUpdate && {
        bio: vnode.state.profileUpdate.bio,
      }),
      ...('avatarUrl' in vnode.state.profileUpdate && {
        avatarUrl: vnode.state.profileUpdate.avatarUrl,
      }),
      ...('website' in vnode.state.profileUpdate && {
        website: vnode.state.profileUpdate.website,
      }),
      jwt: app.user.jwt,
    }).catch(() => {
  ...
  handleInputChange = (vnode, value, formField: InputFormField) => {
    if (formField === InputFormField.Email) {
      ...
      else delete vnode.state.profileUpdate.email;
    }
    ...
    if (formField === InputFormField.ProfileImage) {
      ...
      else delete vnode.state.profileUpdate.avatarUrl;
    }
    ...
    if (formField === InputFormField.Website) {
      ...
      else delete vnode.state.profileUpdate.website;
    }
  };
        ...
        <div className="EditProfilePage">
          ...
          <div className="edit-pane">
            ...
            <CWButton
              ...
              className={
                vnode.state.saved ? 'save-button confirm' : 'save-button'
              }
            />
