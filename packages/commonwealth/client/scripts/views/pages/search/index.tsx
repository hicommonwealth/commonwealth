...
const SearchPage = () => {
  ...
  const community =
    queryParams.communityScope || app.activeChainId() || 'all_chains';
  ...
  const handleSearchAllCommunities = () => {
    const newQueryParams = new URLSearchParams(urlQueryParams.toString());
    newQueryParams.set('communityScope', 'all_chains');
    navigate({
      pathname: location.pathname,
      search: `?${newQueryParams.toString()}`,
    });
          ...
          {!isLoading && (
            <>
              <CWText className="search-results-caption">
                ...
                {scopeText}
                {community !== 'all_chains' && !app.isCustomDomain() && (
                  <a
                    href="#"
