...
const renderBio = (bio: string, isExpanded: boolean) => {
  if (!bio) return DefaultProfileBio;
...
class NewProfileHeader
  implements m.Component<ProfileHeaderAttrs, ProfileHeaderState>
{
  oninit(vnode) {
          ...
          <section className="profile-name-and-bio">
            ...
            {profile?.bio?.length > maxBioCharCount ? (
              <div
                className="read-more"
                onclick={() => {
                  vnode.state.isBioExpanded = !vnode.state.isBioExpanded;
                }}
              >
                <p>{!vnode.state.isBioExpanded ? 'Read More' : 'Read Less'}</p>
              </div>
            ) : (
              <div />
            )}
