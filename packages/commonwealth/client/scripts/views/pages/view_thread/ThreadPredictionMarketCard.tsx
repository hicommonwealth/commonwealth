...
import './prediction_market_cards.scss';
...
export const ThreadPredictionMarketCard = ({
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
              await cancelMarket({ prediction_market_id: market.id });
            } catch (err) {
  ...
  const formatCollateral = (amount: string) => {
    try {
  ...
  const isThreadAuthor =
    ...
    user.activeAccount?.community?.id === thread.community_id;
