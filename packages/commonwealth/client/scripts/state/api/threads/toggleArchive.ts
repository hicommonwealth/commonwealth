...
const toggleThreadArchive = async ({
  ...
  return await axios.post(`${app.serverUrl()}/threads/${threadId}/${!isArchived ? 'archive' : 'unarchive'}`, {
    data: {
      jwt: app.user.jwt,
      chain_id: chainId,
    } as any,
...
const toggleThreadArchiveMutation = ({ chainId, threadId }: ToggleThreadArchiveMutationProps) => {
