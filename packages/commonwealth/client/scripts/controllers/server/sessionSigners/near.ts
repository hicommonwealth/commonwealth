...
export class NEARSessionController implements ISessionController {
  ...
  async getOrCreateAddress(chainId: string, fromAddress: string): Promise<string> {
    return (await this.getOrCreateSigner(chainId, fromAddress)).getPublicKey().toString();
  }
  ...
  private async getOrCreateSigner(chainId: string, fromAddress: string): Promise<KeyPairEd25519> {
    ...
    try {
      ...
      this.signers[chainId][fromAddress] = new nearApiUtils.KeyPairEd25519(secretKey);
