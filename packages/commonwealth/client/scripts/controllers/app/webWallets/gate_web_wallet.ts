...
class GateWebWalletController implements IWebWallet<string> {
  // GETTERS/SETTERS
  ...
  public readonly name = WalletId.Gate;
  public readonly label = 'Gate Wallet';
  ...
  public async enable(forceChainId?: string) {
    console.log('Attempting to enable Gate Wallet');
    this._enabling = true;
    try {
      // default to ETH
      ...
      let ethereum = window.gatewallet;

      if (window.gatewallet.providers?.length) {
        window.gatewallet.providers.forEach(async (p) => {
          if (p.isGateWallet) ethereum = p;
        });
      }
      ...
      this._web3 =
        config?.TEST_EVM_ETH_RPC !== 'e2e-test'
          ...
          : {
              givenProvider: window.gatewallet,
              eth: {
                getAccounts: async () => {
      ...
      try {
        ...
        // This error code indicates that the chain has not been added to Gate Wallet.
      ...
      if (this._accounts.length === 0) {
        throw new Error('Gate Wallet fetched no accounts');
      }
      ...
      let errorMsg = `Failed to enable Gate Wallet: ${error.message}`;
      if (error.code === 4902) {
        errorMsg = `Failed to enable Gate Wallet: Please add chain ID ${app?.chain?.meta?.ChainNode?.eth_chain_id || 0}`;
      }
...
export default GateWebWalletController;
