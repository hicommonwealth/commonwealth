...
class OkxWebWalletController implements IWebWallet<string> {
  ...
  public async enable(forceChainId?: string) {
    // TODO: use RPC API to switch active chain according to currently active node, if one exists
    console.log('Attempting to enable OKX Wallet');
    this._enabling = true;
    try {
      // default to ETH
      ...
      if (this._accounts.length === 0) {
        throw new Error('OKX Wallet fetched no accounts');
      }
      ...
      let errorMsg = `Failed to enable OKX Wallet: ${error.message}`;
      if (error.code === 4902) {
        errorMsg = `Failed to enable OKX Wallet: Please add chain ID ${app?.chain?.meta?.ChainNode?.eth_chain_id || 0}`;
      }
