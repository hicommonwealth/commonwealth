...
class BitgetWebWalletController extends SuiWebWalletController {
  public name = WalletId.bitgetWallet;
  public label = 'Bitget Wallet';

  override get available() {
    const availableWallets = getWallets().get();
    // BitGet is a specific wallet that should be available
    const wallet = availableWallets.find((w) =>
      w.name.toLowerCase().includes('bitget'),
    );
    if (wallet) {
      return true;
    }
  ...
  override getSuiWalletProvider(): Wallet | null {
    ...
    // Prioritize BitGet wallet if available
    const walletPriority = ['bitget'];
...
export default BitgetWebWalletController;
