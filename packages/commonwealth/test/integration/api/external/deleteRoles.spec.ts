...
describe('postRoles Tests', () => {
  ...
  it('fail due to user not owning address', async () => {
    ...
    chai.assert.equal(await models.Role.count({ where: { id: { [Op.in]: [smallestId - 2] } } }), 0);

    let resp = await post('/api/roles', {
      jwt: jwtToken,
      roles: [
        { id: smallestId - 2, community_id: testChains[0].id, address_id: testAddresses[2].id },
      ]
    });

    chai.assert.equal(resp.result.error, '');
    chai.assert.equal(await models.Role.count({ where: { id: { [Op.in]: [smallestId - 2] } } }), 1);

    resp = await del('/api/roles', {jwt: jwtTokenUser2, ids: [smallestId - 2] }, true);

    chai.assert.deepEqual(resp.result.error.unownedAddresses, [{ addressId: -3 }]);
    chai.assert.equal(await models.Role.count({where: {id: {[Op.in]: [smallestId - 2]}}}), 1);
  });
