...
describe('putComments Tests', () => {
  ...
  it('fail due to user not owning address', async () => {
    ...
    const resp = await put('/api/comments', {
      jwt: jwtToken,
      comments: [
        { id: smallestId - 3, address_id: testAddresses[1].id, community_id: testChains[0].id, text: 'test' },
        { id: smallestId - 4, address: testAddresses[1].address, community_id: testChains[0].id, text: 'test' }
      ]
    });
