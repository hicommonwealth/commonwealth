...
describe('postRules Tests', () => {
  it('add entities to db', async () => {
    ...
    chai.assert.equal(await models.Rule.count({ where: { id: { [Op.in]: [smallestId - 1, smallestId - 2] } } }), 0);
    ...
    chai.assert.equal(await models.Rule.count({ where: { id: { [Op.in]: [smallestId - 1, smallestId - 2] } } }), 2);
  });
