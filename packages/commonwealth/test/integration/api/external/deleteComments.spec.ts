...
import { testAddresses, testChains, testComments, testUsers } from './dbEntityHooks.spec';
...
describe('putComments Tests', () => {
  ...
  it('fail due to user not owning address', async () => {
    ...
    chai.assert.equal(await models.Comment.count({where: {id: {[Op.in]: [smallestId - 3, smallestId - 4]}}}), 0);

    let resp = await put('/api/comments', {
      jwt: jwtToken,
      comments: [
        {id: smallestId - 3, address_id: testAddresses[0].id, community_id: testChains[0].id, text: "test"},
        {id: smallestId - 4, address: testAddresses[0].address, community_id: testChains[0].id, text: "test"}
      ]
    });

    chai.assert.equal(resp.result.error, '');
    chai.assert.equal(await models.Comment.count({where: {id: {[Op.in]: [smallestId - 3, smallestId - 4]}}}), 2);

    resp = await del('/api/comments', {jwt: jwtTokenUser2, ids: [smallestId - 3, smallestId - 4] }, true);
