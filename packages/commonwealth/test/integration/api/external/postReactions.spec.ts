...
import { testAddresses, testChains, testComments, testReactions, testUsers } from './dbEntityHooks.spec';
...
describe('postReactions Tests', () => {
  ...
  it('fail due to user not owning address', async () => {
    ...
    const resp = await post('/api/reactions', {
      jwt: jwtToken,
      reactions: [
        { id: smallestId - 1, address_id: testAddresses[1].id, community_id: testChains[0].id, reaction: 'like' },
        { id: smallestId - 2, address: testAddresses[1].address, community_id: testChains[0].id, reaction: 'like' }
      ]
    });
