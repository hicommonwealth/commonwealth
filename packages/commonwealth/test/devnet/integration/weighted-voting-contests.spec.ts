...
describe('Weighted Voting Contests E2E Integration Test', () => {
  ...
  beforeAll(async () => {
    ...
    rpcUrl = process.env.FORK_URL as string;

    // Set up blockchain SDK instances
    namespaceFactory = new NamespaceFactory();
    communityStake = new CommunityStake();
  ...
  test('should correctly calculate contest scores with weighted voting', async () => {
    ...
    for (const action of contestActions) {
      ...
      expect(Number(currentWeight)).toBeLessThanOrEqual(Number(previousWeight));
