    ...
    passport.use(
      new MagicStrategy({ passReqToCallback: true }, async (req, user, cb) => {
        ...
        if (!existingUser && !registrationChain?.base) {
          ...
          // check if this is a new signup or a login
          const existingUser = await models.User.scope(
            'withPrivateData'
          ).findOne({
            where: {
              email: userMetadata.email,
          ...
          if (!existingUser && !registrationChain?.base) {
            ...
            try {
              ...
              if (polkadotResp.body?.status !== 'ok') {
                throw new Error(
                  polkadotResp.body?.message ||
                    'Failed to fetch polkadot address'
                );
              }
              const polkadotRespAddress =
                polkadotResp.body?.data?.public_address;
              ...
              if (registrationChain?.base && registrationChain.ss58_prefix) {
                polkadotAddress = encodeAddress(
                  polkadotRespAddress,
                  registrationChain.ss58_prefix
                ); // edgeware SS58 prefix
              } else {
