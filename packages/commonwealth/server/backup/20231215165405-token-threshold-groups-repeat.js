...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.sequelize.transaction(async (t) => {
      // truncate Memberships and Groups
      await queryInterface.sequelize.query(`DELETE FROM "Memberships"`);
      ...
      for (const topic of topics) {
        ...
        if (!source) {
          console.log(
            `skipped topic ${topic_id} due to unsupported network ${network}`,
          );
          continue;
        }
        const metadata = {
        ...
        const existingGroupId = await findGroup(
          queryInterface,
          topic_chain_id,
          requirements,
        );
        if (existingGroupId) {
          ...
          const createdGroupId = await insertGroup(
            queryInterface,
            topic_chain_id,
            metadata,
            requirements,
...
async function insertGroup(
  ...
  `;
  const [[{ id: createdGroupId }]] = await queryInterface.sequelize.query(
    insertGroupQuery,
    {
      replacements: {
        community_id,
