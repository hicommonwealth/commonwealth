...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    try {
      await queryInterface.sequelize.transaction(async (transaction) => {
        ...
        await queryInterface.sequelize.query(
          `
            INSERT INTO "DisabledCommunityAlerts" (user_id, community_id)
        ...
        await queryInterface.sequelize.query(
          `
          CREATE OR REPLACE FUNCTION insert_disabled_community_alert()
          RETURNS TRIGGER AS $$
          BEGIN
            IF NOT EXISTS (
              SELECT 1 FROM "DisabledCommunityAlerts"
              WHERE user_id = NEW.subscriber_id
                AND community_id = NEW.community_id
            ) THEN
              INSERT INTO "DisabledCommunityAlerts" (user_id, community_id, created_at, updated_at)
        ...
        await queryInterface.sequelize.query(
          ...
          BEGIN
            IF OLD.category_id IN ('chain-event', 'snapshot-proposal') THEN
              DELETE FROM "DisabledCommunityAlerts"
        ...
        await queryInterface.sequelize.query(
          `
          CREATE TRIGGER "OldSubscriptionsDeleteTrigger"
        ...
        await queryInterface.sequelize.query(
          ...
          BEGIN
            IF NEW.category_id IN ('chain-event', 'snapshot-proposal') THEN
              PERFORM insert_disabled_community_alert();
        ...
        await queryInterface.sequelize.query(
          `
          CREATE TRIGGER "OldSubscriptionsInsertTrigger"
  ...
  down: async (queryInterface, Sequelize) => {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      await queryInterface.sequelize.query(
        `
        DROP TRIGGER "ThreadSubscriptionTrigger" ON "ThreadSubscriptions";
        DROP TRIGGER "ThreadSubscriptionDeleteTrigger" ON "ThreadSubscriptions";
        DROP TRIGGER "CommentSubscriptionTrigger" ON "CommentSubscriptions";
        DROP TRIGGER "CommentSubscriptionDeleteTrigger" ON "CommentSubscriptions";
        DROP TRIGGER "DisabledCommunityAlertTrigger" ON "DisabledCommunityAlerts";
      `,
        { transaction },
      );
      ...
      await queryInterface.dropTable('DisabledCommunityAlerts', {
        transaction,
      });
    });
  },
