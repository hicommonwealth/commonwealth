...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    let reactions = await queryInterface.sequelize.query(`SELECT (id, object_id) FROM "OffchainReactions"`);
    ...
    await Promise.all(Object.entries(updatedReactions).map(async (rxn) => {
      const rxnId = rxn[0];
      const objId = rxn[1];
      const thread = await queryInterface.sequelize.query(`SELECT id FROM "OffchainThreads" WHERE id=${Number(objId)}`);
      if (thread[0][0] && thread[0][0].id) {
        const query = `UPDATE "OffchainReactions" SET thread_id=${objId} WHERE id=${Number(rxnId)}`;
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    let reactions = await queryInterface.sequelize.query(`SELECT (id, thread_id) FROM "OffchainReactions"`);
    reactions = reactions[0].map((rxn) => {
    ...
    await Promise.all(Object.entries(updatedReactions).map(async (rxn) => {
      ...
      const thread = await queryInterface.sequelize.query(`SELECT id FROM "OffchainThreads" WHERE id=${Number(threadId)}`);
      if (thread[0][0] && thread[0][0].id) {
        const query = `UPDATE "OffchainReactions" SET object_id='discussion_${threadId}' WHERE id=${Number(rxnId)}`;
        await queryInterface.sequelize.query(query);
      }
    }));
