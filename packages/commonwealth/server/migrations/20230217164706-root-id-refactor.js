    ...
    await queryInterface.sequelize.transaction(async (t) => {
      ...
      threadComments[0].forEach(c => {
        updateQueries.push(queryInterface.sequelize.query(
          `UPDATE "Comments" SET "root_id" = ${c.root_id.split('_')[1]} WHERE "id"=${c.id}`,
          { transaction: t }
        ));
      });
      ...
      const rootToThreadMap = new Map(inserted.map(thread => {
        const urlParts = thread.url.split('/');
        let rootId = urlParts[urlParts.length - 2] + '_' + urlParts[urlParts.length - 1];
        if (urlParts[urlParts.length - 2] === 'proposal') {
          rootId = `cosmos${urlParts[urlParts.length - 2]}` + '_' + urlParts[urlParts.length - 1];
        }
      ...
      proposalComments[0].forEach(c => {
        ...
        if (c.root_id.includes('councilcandidate') || c.root_id.includes('councilmotion')) {
          updateQueries.push(queryInterface.sequelize.query(
        ...
        updateQueries.push(queryInterface.sequelize.query(
          `UPDATE "Comments" SET "root_id" = ${rootToThreadMap.get(c.root_id)} WHERE "id"=${c.id}`,
          { transaction: t }
        ));
      });
