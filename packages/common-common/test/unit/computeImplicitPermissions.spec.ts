...
describe('computeImplicitPermissions() unit tests', () => {
  ...
  it('should correctly computeImplicitPermissions for an action', () => {
  ...
  it('should correctly computePermissions for an action and its implicit permissions with one denial that is overwritten', () => {
    ...
    // eslint-disable-next-line no-bitwise
    overwrite_admin.deny = BigInt(1) << BigInt(Action.VIEW_THREADS);
