...
describe('computeImplicitPermissions() unit tests', () => {
  ...
  it('should correctly computeImplicitPermissions for Allowing createThread action with one role overwrite', () => {
    overwrite_moderator.deny = BigInt(1) << BigInt(Action.CREATE_THREAD);
    const permission = computeImplicitPermissions(overwrite_moderator.deny, Action.CREATE_THREAD, true);
    assert.isTrue(isPermitted(permission, Action.CREATE_THREAD));
    assert.isTrue(isPermitted(permission, Action.VIEW_COMMENTS));
  });

  it('should correctly computeImplicitPermissions for Denying createThread action with one role overwrite', () => {
    overwrite_moderator.deny = BigInt(1) << BigInt(Action.CREATE_THREAD) | BigInt(1) << BigInt(Action.VIEW_COMMENTS);
    const permission = computeImplicitPermissions(overwrite_moderator.deny, Action.CREATE_THREAD, false);
