...
class StorageFetcher extends interfaces_1.IStorageFetcher {
    ...
    constructor(_api, chain) {
        super(_api);
        this._api = _api;
        this.log = logging_1.factory.getLogger((0, logging_1.addPrefix)(__filename, [interfaces_1.SupportedNetwork.Compound, chain]));
    }
    _currentBlock;
    async fetchOne(id) {
    ...
    async fetch(range) {
        ...
        const createdCwEvents = await Promise.all(proposalCreatedEvents.map((evt) => (0, enricher_1.Enrich)(this._api, evt.blockNumber, types_1.EventKind.ProposalCreated, evt)));
        const voteCastEvents = await this._api.queryFilter((0, types_1.isGovernorAlpha)(this._api)
        ...
        const voteCwEvents = await Promise.all(voteCastEvents.map((evt) => (0, enricher_1.Enrich)(this._api, evt.blockNumber, types_1.EventKind.VoteCast, evt)));
        let queuedCwEvents = [];
        try {
            ...
            queuedCwEvents = await Promise.all(proposalQueuedEvents.map((evt) => (0, enricher_1.Enrich)(this._api, evt.blockNumber, types_1.EventKind.ProposalQueued, evt)));
        }
        catch (e) {
        ...
        const cancelledCwEvents = await Promise.all(proposalCanceledEvents.map((evt) => (0, enricher_1.Enrich)(this._api, evt.blockNumber, types_1.EventKind.ProposalCanceled, evt)));
        ...
        const executedCwEvents = await Promise.all(proposalExecutedEvents.map((evt) => (0, enricher_1.Enrich)(this._api, evt.blockNumber, types_1.EventKind.ProposalExecuted, evt)));
        return [
            ...createdCwEvents,
            ...voteCwEvents,
            ...queuedCwEvents,
