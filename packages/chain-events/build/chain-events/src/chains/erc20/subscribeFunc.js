...
async function createApi(ethNetworkUrl, tokenAddresses, tokenNames, retryTimeMs = 10 * 1000) {
    const log = logging_1.factory.getLogger((0, logging_1.addPrefix)(__filename, [interfaces_1.SupportedNetwork.ERC20]));
    for (let i = 0; i < 3; ++i) {
        try {
            const provider = await (0, eth_1.createProvider)(ethNetworkUrl, interfaces_1.SupportedNetwork.ERC20);
            ...
            for (const [contract, tokenName] of underscore_1.default.zip(tokenContracts, tokenNames)) {
                try {
                    ...
                    const totalSupply = new bn_js_1.default((await contract.totalSupply()).toString());
    ...
    throw new Error(`[${interfaces_1.SupportedNetwork.ERC20}]: Failed to start the ERC20 listener for ${tokenAddresses} at ${ethNetworkUrl}`);
}
