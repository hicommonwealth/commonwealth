...
class Processor extends interfaces_1.IEventProcessor {
    ...
    async process(event, tokenName) {
        const log = logging_1.factory.getLogger((0, logging_1.addPrefix)(__filename, [interfaces_1.SupportedNetwork.ERC20, tokenName]));
        const kind = (0, type_parser_1.ParseType)(event.event);
        if (!kind)
            return [];
        try {
            const cwEvent = await (0, enricher_1.Enrich)(this._api, event.blockNumber, kind, event, this._enricherConfig);
