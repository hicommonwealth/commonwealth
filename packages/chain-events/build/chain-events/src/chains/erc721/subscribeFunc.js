...
async function createApi(ethNetworkUrl, tokenAddresses, tokenNames, retryTimeMs = 10 * 1000) {
    const log = logging_1.factory.getLogger((0, logging_1.addPrefix)(__filename, [interfaces_1.SupportedNetwork.ERC721]));
    for (let i = 0; i < 3; ++i) {
        try {
            const provider = await (0, eth_1.createProvider)(ethNetworkUrl, interfaces_1.SupportedNetwork.ERC721);
