...
class Processor extends interfaces_1.IEventProcessor {
     ...
     */
    async process(event) {
        const log = logging_1.factory.getLogger((0, logging_1.addPrefix)(__filename, [interfaces_1.SupportedNetwork.Moloch, this.chain]));
        const kind = (0, type_parser_1.ParseType)(this._version, event.event, this.chain);
        if (!kind)
            return [];
        try {
            const cwEvent = await (0, enricher_1.Enrich)(this._version, this._api, event.blockNumber, kind, event);
            return [cwEvent];
        }
        catch (e) {
