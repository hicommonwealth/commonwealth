...
class StorageFetcher extends interfaces_1.IStorageFetcher {
    ...
    constructor(_api, _version, _dater, chain) {
        ...
        this._dater = _dater;
        this.log = logging_1.factory.getLogger((0, logging_1.addPrefix)(__filename, [interfaces_1.SupportedNetwork.Moloch, chain]));
    }
    _periodDuration; // 1 period in seconds
    _summoningTime; // starting time of contract
    _abortPeriod;
    _votingPeriod;
    _gracePeriod;
    _currentBlock;
    ...
    async _eventsFromProposal(index, proposal, startTime, startBlock) {
        const events = [];
    ...
    async fetchOne(id) {
        ...
        try {
            proposal =
                this._version === 1
                    ? await this._api.proposalQueue(id)
                    : await this._api.proposals(id);
        }
        catch (e) {
        ...
        const events = await this._eventsFromProposal(+id, proposal, proposalStartingTime, proposalStartBlock);
        return events;
    }
    /**
     ...
     */
    async fetch(range, fetchAllCompleted = false) {
        await this._initConstants();
        if (!this._currentBlock) {
            this.log.error('Failed to fetch current block! Aborting fetch.');
            return [];
        ...
        for (let i = 0; i < queueLength; i++) {
            ...
            const proposalIndex = this._version === 1
                ? queuePosition
                : +(await this._api.proposalQueue(queuePosition));
            // fetch actual proposal
            ...
            if (proposalStartBlock >= range.startBlock &&
                ...
                const events = await this._eventsFromProposal(proposalIndex, proposal, proposalStartingTime, proposalStartBlock);
                results.push(...events);
                nFetched += 1;
