...
class Poller extends interfaces_1.IEventPoller {
    ...
    async poll(range, maxRange = 500) {
        ...
        const hashes = await Promise.all(blockNumbers.map((number) => this._api.rpc.chain.getBlockHash(number)));
        ...
        const blocks = await Promise.all(nonZeroHashes.map(async (hash) => {
     ...
     */
    async archive(range, batchSize = 500, processBlockFn = null) {
