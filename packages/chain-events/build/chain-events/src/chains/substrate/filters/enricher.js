...
async function Enrich(api, blockNumber, kind, rawData, config = {}) {
    const extractEventData = async (event) => {
        switch (kind) {
            ...
            case types_2.EventKind.Offence: {
                ...
                const offenceDetails = await api.query.offences.reports.multi(reportIds);
                const allOffenders = offenceDetails.map((offence) => {
                    return offence.isSome ? offence.unwrap().offender[0] : null;
                });
                const offenders = (0, lodash_1.filter)(allOffenders, null);
                return {
                    data: {
                        kind,
            ...
            case types_2.EventKind.NewSession: {
                ...
                const validatorEraPoints = await (0, currentPoint_1.currentPoints)(api, currentEra, hash, validators);
            ...
            case types_2.EventKind.Unbonded: {
                ...
                const controllerOpt = await api.query.staking.bonded.at(hash, stash);
            ...
            case types_2.EventKind.DemocracyStarted: {
                ...
                const infoOpt = await api.query.democracy.referendumInfoOf(referendumIndex);
                if (!infoOpt.isSome) {
            ...
            case types_2.EventKind.NewTip: {
                ...
                return {
                    data: {
                        ...
                        reason: (0, util_1.hexToString)(reasonOpt.unwrap().toString()),
                        who: who.toString(),
            ...
            case types_2.EventKind.ElectionNewTerm: {
                ...
                const round = await api.query[section].electionRounds();
                return {
                    data: {
                        kind,
                        round: +round,
            ...
            case types_2.EventKind.CollectiveProposed: {
                ...
                const proposalOpt = await api.query[event.section].proposalOf(hash);
                if (!proposalOpt.isSome) {
            ...
            case types_2.EventKind.SignalingNewProposal: {
                ...
                const proposalInfoOpt = await api.query.signaling.proposalOf(hash);
                if (!proposalInfoOpt.isSome) {
                    throw new Error(`unable to fetch signaling proposal info`);
                }
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const voteInfoOpt = await api.query.voting.voteRecords(proposalInfoOpt.unwrap().vote_id);
                ...
                if (judgementInfo.length > 0) {
                    ...
                    judgements.push(...judgementInfo.map(([id, judgement]) => {
                        ...
                        return [
                            ...
                            (0, types_2.parseJudgement)(judgement),
                        ];
                    }));
                }
                return {
            ...
            case types_2.EventKind.JudgementGiven: {
                ...
                const judgement = (0, types_2.parseJudgement)(judgementTuple[1]);
                return {
                    data: {
                        kind,
                        who: who.toString(),
    ...
    const extractExtrinsicData = async (extrinsic) => {
        switch (kind) {
            ...
            case types_2.EventKind.TreasuryBountyExtended: {
                ...
                return {
                    data: {
                        kind,
                        bountyIndex: +idx,
                        remark: (0, util_1.hexToString)(remark.toString()),
                    },
                };
            }
            default: {
    ...
    const eventData = await ((0, types_2.isEvent)(rawData)
        ? extractEventData(rawData)
        : extractExtrinsicData(rawData));
    return eventData
        ? { ...eventData, blockNumber, network: interfaces_1.SupportedNetwork.Substrate }
        : null;
}
