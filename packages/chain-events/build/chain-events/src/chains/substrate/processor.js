...
class Processor extends interfaces_1.IEventProcessor {
     ...
     */
    async process(block) {
        const log = logging_1.factory.getLogger((0, logging_1.addPrefix)(__filename, [
        ...
        const applyFilters = async (data) => {
            const section = (0, types_1.isEvent)(data) ? data.section : data.method.section;
            const method = (0, types_1.isEvent)(data) ? data.method : data.method.method;
            const kind = (0, type_parser_1.ParseType)(block.versionName, block.versionNumber, section, method);
            if (kind !== null) {
                try {
                    const result = await (0, enricher_1.Enrich)(this._api, blockNumber, kind, data, this._enricherConfig);
                    return result;
                }
                catch (e) {
