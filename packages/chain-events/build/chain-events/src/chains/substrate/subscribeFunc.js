 ...
 */
async function createApi(url, typeOverrides = {}, chain) {
    const log = logging_1.factory.getLogger((0, logging_1.addPrefix)(__filename, [interfaces_1.SupportedNetwork.Substrate, chain]));
    for (let i = 0; i < 3; ++i) {
...
const subscribeEvents = async (options) => {
    ...
    const processBlockFn = async (block) => {
    ...
    if (archival) {
        ...
        const offlineRange = { startBlock: startBlock ?? 0 };
