    ...
    async safePublish(publishData, objectId, publication, DB) {
        ...
        try {
            ...
            if (e instanceof RabbitMQControllerError) {
                ...
                this.rollbar?.warn(`RepublishMessages job failure for message: ${JSON.stringify(publishData)} to ${publication}.`, e);
            }
            else {
                log.error(`Sequelize error occurred while setting queued to -1 for ${DB.model.getTableName()} with id: ${objectId}`, e);
                this.rollbar?.warn(`Sequelize error occurred while setting queued to -1 for ${DB.model.getTableName()} with id: ${objectId}`, e);
            }
        }
    }
    async shutdown() {
