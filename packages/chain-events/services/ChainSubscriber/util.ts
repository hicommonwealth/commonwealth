...
/**
 * This function iterates through the substrate chains and fetches on-chain
 * identities for each identity of said chain in the database identityCache
 * @param allChains An array containing all the chains pulled from the database
 * @param listenerInstances An object containing all the currently active listener instances
 * @param pool A PG pool instance used by the discoverReconnectRange function
 * @param producer An instance of RabbitMqHandler used to publish identity events
 * @param rollbar An instance of rollbar for error reporting
 */
async function fetchSubstrateIdentities(
  allChains: ChainAttributes[],
  listenerInstances: IListenerInstances,
  pool: Pool,
  producer: RabbitMqHandler,
  ...
  for (const chain of allChains) {
    ...
    let listener: SubstrateEvents.Listener;
    if (!listenerInstances[chain.id]) {
      ...
      listener = <SubstrateEvents.Listener>listenerInstances[chain.id];
    }
    ...
    try {
      ...
      identityEvents = await listener.storageFetcher.fetchIdentities(
        identitiesToFetch
      );
    } catch (error) {
    ...
    // augment the events with the chain id and publish to the identity queue
    for (const event of identityEvents) {
      event.chain = chain.id; //
