...
import {
  ...
  RascalExchanges,
  RascalQueues,
  RascalRoutingKeys, isRmqMsgCreateEntityCUD
...
describe("Tests for the ChainEventsConsumer service", () => {
  ...
  it("Should create an entity in the database and push to the entity cud queue", async () => {
    const max_proposal_id = (<any>(await models.sequelize.query(`
  ...
  it("Should re-queue messages that have failed to queue less than 5 times - RepublishMessages");
  it("Should send messages to a dead letter queue when a message fails to be queued 5 times");
});
