...
import {RascalPublications, RascalSubscriptions} from "common-common/src/rabbitmq/types";
...
describe('Integration tests for Compound Bravo', () => {
  ...
  describe('Tests the ERC20 event listener using the chain subscriber', async () => {
    ...
    it('Should capture transfer events', async () => {
      ...
      events['transfer'] = msg;
      expect(rmq.queuedMessages[RascalSubscriptions.ChainEvents].length).to.equal(1, 'Should have captured 1 transfer event');
      eventMatch(msg, 'transfer', chain_id, null, transferAmount);
    });

    it('Should capture approval events', async () => {
      await sdk.approveErc20(BUSDAddress, randomWallet,  Web3.utils.toWei(transferAmount));
      await delay(10000);

      const msg = rmq.queuedMessages[RascalSubscriptions.ChainEvents][1];
      events['approval'] = msg;
      expect(rmq.queuedMessages[RascalSubscriptions.ChainEvents].length).to.equal(2, 'Should have captured 1 approval event');
  ...
  describe('Tests for processing ERC20 events with the consumer', async () => {
        ...
        where: {
          ...
          event_data: {
            [Op.and]: [
              ...
              Sequelize.literal(`event_data->>'value' = '${Web3.utils.toWei(transferAmount)}'`),
            ]
          },
          block_number: events['transfer'].blockNumber,
        }
      });
      expect(transferEvent, 'The transfer event should be in the database').to.exist;
    });
    ...
    it('Should process approval events', async () => {
      ...
      expect(approvalEvent, 'The approval event should be in the database').to.exist;
    });
  });
