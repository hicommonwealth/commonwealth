...
class Producer {
    create() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const connection = new Amqp.Connection(config_1.rabbitMQ.url);
                this.channel = connection.declareQueue("snapshot_queue", {
                    durable: true,
                });
                this.exchange = connection.declareExchange(config_1.rabbitMQ.exchangeName, "direct", { durable: true });
                this.channel.bind(this.exchange);
                yield connection.completeConfiguration();
            }
            catch (err) {
                console.log(err);
            }
