...
prettier --write ../libs/api-client/package.json ../packages/commonwealth/external-api-config.json

# Check for changes in package.json or external-api-config.json files and commit the changes if any
changed_files=$(git diff --name-only ../libs/api-client/package.json ../packages/commonwealth/external-api-config.json)
