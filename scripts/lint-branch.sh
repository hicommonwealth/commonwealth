...
then
    ...
    if [ -n "$FAIL_WARNINGS" ]; then
        NODE_OPTIONS="--max-old-space-size=8192" eslint --plugin import --max-warnings=0 $LINES --no-ignore
    else
        NODE_OPTIONS="--max-old-space-size=8192" eslint --plugin import $LINES
    fi
fi
