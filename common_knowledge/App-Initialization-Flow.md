...
9. If `selectChain()` (`/helpers/chain.ts`) is fired, per step #8:
    ...
    8. If the user is logged in, his addresses for the initialized community are set as his current active addresses, and the `SelectCommunity` command is invoked to update his last visited community.
