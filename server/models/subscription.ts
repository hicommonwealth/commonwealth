...
export interface SubscriptionInstance
  extends Sequelize.Model<SubscriptionAttributes>,
...
export default (
  ...
  Subscription.emitNotifications = async (
      ...
      notification = await models.Notification.create(
        isChainEventData
          ? {
              ...
              chain_id: (<IChainEventNotificationData>notification_data)
                .chain_id,
            }
          : {
              notification_data: JSON.stringify(notification_data),
    ...
    if (!notification_data) {
      log.info(
        'Subscription is missing notification data, will not trigger send emails or webhooks'
      );
      return [];
    }
      ...
      if (ids && ids.length > 0) {
        ...
        nReads = sequelize.query(
                    ...
                    `,
          {
            raw: true,
            type: 'RAW',
            replacements: [notification.id, ids, category_id, object_id],
          }
        );
      }
    ...
    nReads = await sequelize.query(
            ...
            `,
      {
        raw: true,
        type: 'RAW',
        replacements: [notification.id, category_id, object_id],
      }
    );
      ...
      subscribers.map(async (subscription) => {
        if (
          ...
          msg.dynamic_template_data.notification.path = `${SERVER_URL}/${
            (<IChainEventNotificationData>notification_data).chainEventType
              .chain
    ...
    log.info(`>>>>>>>>>>>>>>>>>>>>>>>>>> Total query time ${Date.now() - startTime}`);
    return notification;
  };
  ...
  Subscription.associate = (models) => {
    ...
    models.Subscription.belongsTo(models.OffchainThread, {
      foreignKey: 'offchain_thread_id',
      targetKey: 'id',
    });
    models.Subscription.belongsTo(models.OffchainComment, {
    ...
    models.Subscription.belongsTo(models.ChainEntity, {
