...
export default async function (models, chain?: string): Promise<void> {
  ...
  for (const node of nodes) {
    ...
    const profiles: OffchainProfileInstance[] =
      await models.OffchainProfile.findAll({
        include: [
          {
    ...
    log.info(
      `Fetching identities on chain ${node.chain} at url ${node.url}...`
    );
    ...
    try {
      ...
      log.info(
        `Writing identities for chain ${node.chain} back to db... (count: ${identityEvents.length})`
      );
