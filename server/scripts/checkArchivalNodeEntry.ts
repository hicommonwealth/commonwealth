...
const ArchivalNodeDBEntry = async (models) => {
  ...
  if (!archivalNodeData) {
    try {
      ...
      console.log(` DB entry created for chain-events version ${CHAIN_EVENTS_VERSION} 
      with starting block number ${process.env.START_BLOCK}`);
    } catch (err) {
      console.error(`Unable to create database entry for archival node execution: ${err.message}`);
      return true;
    }
  } else {
    console.log(`DB entry exists for chain-events version ${CHAIN_EVENTS_VERSION} 
    with starting block number ${process.env.START_BLOCK}`);
    return true;
  }
  return false;
