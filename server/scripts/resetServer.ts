...
const resetServer = (models, closeMiddleware) => {
  ...
  models.sequelize
    ...
    .then(async () => {
      ...
      const furqan = await models.User.create({
        email: "furqan@seedcode.io",
        emailVerified: true,
        isAdmin: true,
        lastVisited: "{}",
      });
      ...
      await models.Subscription.create({
        subscriber_id: furqan.id,
        category_id: NotificationCategories.NewMention,
        object_id: `user-${furqan.id}`,
        is_active: true,
      });
      // Communities
      ...
      await Promise.all(
        EventSupportingChains.map((chain) => initChainEventTypes(chain))
      );
