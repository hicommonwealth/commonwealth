 ...
 */
export function syncPerformanceTester(numSamples, numIter, execFunc, log, outputFile) {
    const results = [];
    for (let i = 0; i < numSamples; i++) {
        ...
        results.push(avg);
        const logString = `[${i + 1}]\tTotal time: ${endTime - startTime},\tAverage Response Time: ${avg}`;
    ...
    const logString = `\n[Overall]\tTotal time: ${sum}\tAverage Response Time: ${sum / results.length}` +
        `\tStandard Deviation: ${getStandardDeviation(results)}\t Median: ${getMedian(results)}`;
    if (log) console.log(logString);
    if (outputFile) {
        fs.writeFile(outputFile, logString, err => {
            if (err) console.error(err);
