...
const getInfuraWeb3Provider = async (network = 'mainnet') => {
  ...
  const provider = new Web3.providers.HttpProvider(
    `https://${network}.infura.io/v3/${DEV_INFURA_TOKEN}`
  );
...
const fetchEvents = async (
  ...
  while (toBlock < latestBlock + blocksPerQuery) {
    const newEvents = await originContract.getPastEvents(eventName, {
      fromBlock,
      toBlock,
    });
    ...
    console.log(
      `Found ${newEvents.length} ${eventName} events (${fromBlock}-${toBlock})`
    );
  }
...
export const updateEvents = (app, models): Promise<number> => {
  return new Promise((resolve) => {
    fs.readFile(
      path.join(__dirname, 'etc/Lockdrop.json'),
      async (error, data) => {
        if (error) {
        ...
        try {
          ...
          const lastSignalEvent = await models.EdgewareLockdropEvent.findOne({
            where: {
              origin: models.sequelize.where(
                models.sequelize.fn('LOWER', models.sequelize.col('origin')),
                'LIKE',
                `%${MAINNET_LOCKDROP}%`
              ),
              name: 'Signaled',
          ...
          const lastSignalBlock = lastSignalEvent
            ? lastSignalEvent.blocknum
            : MAINNET_START_BLOCK;
          const lastLockBlock = lastLockEvent
            ? lastLockEvent.blocknum
            : MAINNET_START_BLOCK;
          ...
          // save events
          const eventsForCreation = [].concat
            .apply([], events)
            .map((event) => ({
              // chain: 'ethereum',
          ...
          const eventsCreated = await models.EdgewareLockdropEvent.bulkCreate(
            eventsForCreation,
            { validate: true }
          );
          ...
          // get balances
          console.log(
            `Getting balances for ${addrKeys.length} new addresses...`
          );
          ...
          const balancesCreated = await models.EdgewareLockdropBalance.bulkCreate(
            balancesForCreation,
            { validate: true }
          );
