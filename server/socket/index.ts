...
export default function (
  ...
  server.on(
    WebsocketEventType.Upgrade,
    (req: express.Request, socket: net.Socket, head) => {
  ...
  wss.on(
    ...
    (ws: AuthWebSocket, req: express.Request) => {
      ...
      ws.on(WebsocketEventType.Message, (message) => {
        ...
        try {
          const payload: IWebsocketsPayload<any> = JSON.parse(
            message.toString()
          );
          if (payload.event === WebsocketMessageType.Heartbeat) {
            ...
            if (payload.jwt) {
              jwt.verify(payload.jwt, JWT_SECRET, async (err, decodedUser) => {
                if (err) {
                  log.info(
                    `received message with malformed JWT: ${payload.jwt}`
                  );
                } else {
  ...
  wss.on(
    ...
    (payload: IWebsocketsPayload<any>, userIds: number[]) => {
      if (logging)
        log.info(
          `Payloading ${JSON.stringify(payload)} to users ${JSON.stringify(
            userIds
          )}`
        );
