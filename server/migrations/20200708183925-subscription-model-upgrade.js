...
module.exports = {
  up: async (queryInterface, DataTypes) => {
    ...
    // add columns
      ...
      const chains = await queryInterface.sequelize.query(`SELECT * FROM "Chains"`, { transaction: t });
      const chainIds = chains[0].map((c) => c.id);
      const subscriptions = await queryInterface.sequelize.query(`SELECT * FROM "Subscriptions";`, { transaction: t });
        ...
        switch (category_id)  {
          ...
          case 'new-comment-creation':  // object_id: "discussion_123"
            ...
            if (entity === 'discussion') {
              ...
              // // associate chain or community
              const comment = await queryInterface.sequelize.query(`SELECT * FROM "OffchainComments" WHERE id=${Number(p_object_id)};`, { transaction: t });
  ...
  down: async (queryInterface, DataTypes) => {
    return queryInterface.sequelize.transaction(async (t) => {
      await Promise.all([
        ...
        queryInterface.removeColumn('Subscriptions', 'community_id', { transaction: t }),
        queryInterface.removeColumn('Subscriptions', 'offchain_thread_id', { transaction: t }),
        queryInterface.removeColumn('Subscriptions', 'offchain_comment_id', { transaction: t }),
        queryInterface.removeColumn('Subscriptions', 'chain_event_type_id', { transaction: t }),
        queryInterface.removeColumn('Subscriptions', 'chain_entity_id', { transaction: t }),
      ]);
    });
  }
};
