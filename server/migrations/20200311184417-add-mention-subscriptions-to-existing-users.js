...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.sequelize.query(
      `INSERT INTO "NotificationCategories" (name, description) VALUES ('new-mention', 'someone @ mentions a user')`
    );
    const category = await queryInterface.sequelize.query(
      `SELECT * FROM "NotificationCategories" WHERE name='new-mention'`
    );
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    await queryInterface.sequelize.query(
      `DELETE FROM "NotificationCategories" WHERE name='new-mention'`
    );
  },
};
