...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const chains = await queryInterface.sequelize.query('SELECT id, network, base FROM "Chains";');
    const promises = [];
    ...
    for (const addr of addresses[0]) {
      ...
      if (ch && ch.base === 'ethereum') {
        if (!Web3.utils.checkAddressChecksum(address)) {
          ...
          const duplicated = await queryInterface.sequelize.query(`SELECT id FROM "Addresses" WHERE address='${checksumAddress}' AND chain='${chain}';`);
