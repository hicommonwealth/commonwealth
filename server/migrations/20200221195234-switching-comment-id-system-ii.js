...
module.exports = {
  up: async (queryInterface, DataTypes) => {
    ...
    comments = comments[0].map((comment) => {
      const [id, parent_id, parent_type] = comment.row.slice(1, comment.row.length - 1).split(',');
    ...
    await Promise.all(Object.entries(updatedComments).map(async (entry) => {
      ...
      await queryInterface.sequelize.query(queryTable + queryValues + queryConditional);
    }));
  ...
  down: async (queryInterface, DataTypes) => {
    ...
    let comments = await queryInterface.sequelize
      .query(`SELECT (id, parent_id, root_id) FROM "OffchainComments"`);
    comments = comments[0].map((comment) => {
      const [id, parent_id, root_id] = comment.row.slice(1, comment.row.length - 1).split(',');
