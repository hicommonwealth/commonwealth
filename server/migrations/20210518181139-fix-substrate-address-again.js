...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    if (removeAddressIds.length) {
      ...
      const reactions = await queryInterface.sequelize.query(`SELECT id FROM "OffchainReactions" WHERE address_id IN (${payload});`);
      if (reactions[0] && reactions[0].length) {
        const reactionIds = reactions[0].map((_) => _.id).join(', ');
        await queryInterface.sequelize.query(`DELETE FROM "OffchainReactions" WHERE comment_id IN (${reactionIds});`);
      }
