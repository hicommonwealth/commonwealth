...
module.exports = {
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.query(
      ...
      const duplicateAddrs = Object.entries(counts).filter(([addr, count]) => count > 1);
      console.log('skipping addresses with conflicting encodings:', duplicateAddrs);
      const migratingAddrs = addresses.filter(a => duplicateAddrs.indexOf(a.address) === -1);

        for (const r of migratingAddrs) {
          try {
            await queryInterface.sequelize.query(`UPDATE "Addresses" SET address = ? WHERE id = ?`, {
