...
const createComment = async (
  ...
  if (chain && chain.type === 'token') {
    ...
    if (isAdmin.length === 0) {
      const userHasBalance = await tokenBalanceCache.hasToken(
        chain.id,
        req.body.address
      );
      if (!userHasBalance)
  ...
  if (parent_id) {
    ...
    if (!parentCommentIsVisibleToUser)
      return next(new Error(Errors.InvalidParent));
  }
  ...
  const cwUrl =
    typeof proposal === 'string'
      ? getProposalUrlWithoutObject(
          prefix,
          finalComment.chain || finalComment.community,
          proposal,
          finalComment
        )
