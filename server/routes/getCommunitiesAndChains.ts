...
const getCommunitiesAndChains = async (
  models: DB,
  req: Request,
  res: Response,
  next: NextFunction
  ...
  const chains: (ChainInstance | OffchainCommunityInstance)[] =
    await models.Chain.findAll(params);
  ...
  const visibleCommunities = communities.filter((community) => {
    if (!community.privacy_enabled) {
      ...
      const userMembership = userRoles.find(
        (role) => role.offchain_community_id === community.id
      );
  ...
  return res.json({
    status: 'Success',
    result: chainsAndCommunities.map((p) => p.toJSON()),
  });
};
