...
export const getLocksForAddress = async (
  userAddress,
  ...
  const results = await Promise.all(
    lockEvents.map(async (event) => {
      const lockStorage = await getLockStorage(
...
export const getSignalsForAddress = async (
  userAddress,
  lockdropContractAddress,
  web3
) => {
  ...
  const results = await Promise.all(
    signalEvents.map(async (event) => {
      const balance = await web3.eth.getBalance(
        event.returnValues.contractAddr,
        8461046
      );
      return {
        ...event,
