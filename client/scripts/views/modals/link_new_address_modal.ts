...
> = {
  view: (vnode) => {
    ...
    return m(
      `.EthereumLinkAccountItem.account-item${
        isPrepopulated ? '.account-item-emphasized' : ''
      }`,
      {
        onclick: async (e) => {
          e.preventDefault();
          ...
          if (app.isLoggedIn()) {
            ...
            if (result.exists) {
              if (result.belongsToUser) {
                ...
                const modalMsg =
                  'Another user owns this address. Remove it from that user and transfer it to yours?';
          ...
          const webWalletSignature = await webWallet.signMessage(
            signerAccount.validationToken
          );

          signerAccount
            .validate(webWalletSignature)
            .then(() => {
...
> = {
  view: (vnode) => {
    ...
    return m(
      `.CosmosLinkAccountItem.account-item${
        isPrepopulated ? '.account-item-emphasized' : ''
      }`,
      {
        onclick: async (e) => {
          ...
          if (!offlineSigner)
            return notifyError('Missing or misconfigured web wallet');
          vnode.state.linking = true;
          m.redraw();
          ...
          ((client as any).signer.signAmino
            ...
            .then(async (signature) => {
              return signerAccount
                .validate(JSON.stringify(signature))
                .then(() => {
                  // return if user signs for two addresses
                  if (linkNewAddressModalVnode.state.linkingComplete) return;
                  linkNewAddressModalVnode.state.linkingComplete = true;
                  return accountVerifiedCallback(signerAccount).then(() =>
                    m.redraw()
                  );
                })
                .catch((err) => {
      ...
      [
        m('.account-item-avatar', [
          m(
            '.account-user',
            m(User, {
              user: app.chain.accounts.get(account.address),
              avatarOnly: true,
              avatarSize: 40,
            })
          ),
...
> = {
  view: (vnode) => {
    ...
    return m(
      `.SubstrateLinkAccountItem.account-item${
        isPrepopulated ? '.account-item-emphasized' : ''
      }`,
      {
        onclick: async (e) => {
  ...
  view: (vnode) => {
        ...
        try {
          ...
          try {
            if (
              ...
              vnode.attrs.joiningCommunity &&
              !app.user.getRoleInCommunity({
                account,
                community: vnode.attrs.joiningCommunity,
              })
            ) {
              await app.user.createRole({
                address: addressInfo,
    ...
    return m('.LinkNewAddressModal', [
      vnode.state.step === LinkNewAddressSteps.Step1VerifyWithWebWallet
            ...
            m('.link-address-step-narrow', [
              app.chain.webWallet?.accounts?.length === 0 &&
                m(Button, {
                  ...
                  rounded: true,
                  disabled:
                    !app.chain.webWallet?.available || // disable if unavailable
                  ...
                  label: !app.chain.webWallet?.available
                    ? 'No wallet detected'
                    : vnode.state.initializingWallet !== false &&
                ...
                m('.accounts-caption', [
                  app.chain.webWallet?.accounts.length === 0
                    ...
                    ? [
                        m(
                          'p.small-text',
                          'To connect with a different account, select it in your wallet, and refresh the page.'
                        ),
                      ]
                    : [
              ...
              m('.accounts-list', [
                app.chain.base === ChainBase.NEAR
                  ? [
                      m(Button, {
                        ...
                        onclick: async (e) => {
                          ...
                          const WalletAccount = (await import('nearlib'))
                            .WalletAccount;
                  ...
                  : app.chain.base === ChainBase.Ethereum
                  ? [
                      app.chain.webWallet?.accounts.map((address) =>
                        m(EthereumLinkAccountItem, {
                          address,
                          targetCommunity,
                          accountVerifiedCallback,
                          errorCallback: (error) => {
        ...
        : vnode.state.step === LinkNewAddressSteps.Step1VerifyWithCLI
        ? m('.link-address-step', [
            linkAddressHeader,
            m('.link-address-step-narrow', [
              app.chain.base === ChainBase.Substrate && [
              ...
              vnode.state.newAddress &&
                m('.link-address-cli-verification', [
                  app.chain.base === ChainBase.Substrate && [
                    ...
                    m(CodeBlock, { clickToSelect: true }, [
                      `echo "${
                        ...
                        vnode.state.isEd25519 ? '--scheme ed25519 ' : ''
                      } "`,
                  ...
                  m(Button, {
                    ...
                    onclick: async (e) => {
                      e.preventDefault();
                      const unverifiedAcct: Account<any> =
                        vnode.state.newAddress;
                      unverifiedAcct
                        .validate(vnode.state.userProvidedSignature)
                        .then(
                          () => {
                            // if no exception was raised, account must be valid
                            accountVerifiedCallback(
                              app.chain.accounts.get(unverifiedAcct.address)
                            );
                          },
                          (err) => {
                    ...
                    label: 'Continue',
                    disabled: !(
                      vnode.state.userProvidedSignature &&
                      (app.chain.base !== ChainBase.Substrate ||
                        vnode.state.secretPhraseSaved)
                    ),
                  }),
                ]),
            ]),
