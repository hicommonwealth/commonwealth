  ...
  view: (vnode) => {
    ...
    return m('.MessageSigningCLIOption', [
      app.chain &&
        ...
        m('.instructions', ['Use subkey to sign this message:']),
      ...
      app.chain &&
        ...
        m(CodeBlock, { clickToSelect: true }, [
          `echo "${account.validationToken}" | subkey ${
            (account as SubstrateAccount).isEd25519 ? '-e ' : ''
          }sign "`,
      ...
      app.chain &&
        app.chain.base === ChainBase.CosmosSDK &&
        m(CodeBlock, { clickToSelect: true }, [`${account.validationToken}`]),
      // action
...
export const getSignatureForMessage = (
  account: Account<any>,
  title?: string,
  method?: string
) => {
  return new Promise((resolve, reject) => {
    ...
    app.modals.create({
      ...
      exitCallback: (data) => {
        complete ? resolve(data) : reject(data);
      },
      data: {
        account,
        title,
