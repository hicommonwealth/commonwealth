...
export const GovernanceSection: m.Component<{ mobile: boolean }, {}> = {
  view: (vnode) => {
    ...
    const showValidators =
      ...
      app.chain?.network !== ChainNetwork.Darwinia;
    const showDelegation = true; // TODO: Conditional
    ...
    const councillorsData: SectionGroupProps = {
      title: 'Councillors',
      contains_children: false,
    ...
    // Delegation
    const delegationData: SectionGroupProps = {
      title: 'Delegates',
      contains_children: false,
      default_toggle: showDelegation
        ? toggle_tree_state['children']['Delegation']['toggled_state']
        : false,
      is_visible: showDelegation,
      is_updated: true,
      is_active: m.route.get() === `/${app.activeChainId()}/delegation`,
      onclick: (e, toggle: boolean) => {
        e.preventDefault();
        setGovernanceToggleTree('children.Delegation.toggled_state', toggle);
    ...
    const sidebarSectionData: SidebarSectionProps = {
      ...
      display_data: governanceGroupData,
      is_active: false,
