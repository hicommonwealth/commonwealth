        ...
        m('.community-label-right', [
          m('.community-name-row', [
            ...
            showStatus === true && [
              ...
              !community.privacyEnabled &&
                m(Icon, { name: Icons.GLOBE, size: 'xs' }),
            ],
          ]),
        ]),
      ]);
...
export const CurrentCommunityLabel: m.Component<{}> = {
  view: (vnode) => {
    ...
    if (selectedNode) {
      ...
      return m(CommunityLabel, {
        community: selectedCommunity.meta,
        showStatus: true,
        link: true,
      });
    } else {
  ...
  view: (vnode) => {
    ...
    const allCommunities = (app.config.communities.getAll() as (
      ...
      .sort((a, b) => {
        ...
        if (a instanceof ChainInfo && app.communities.isStarred(a.id, null))
          return -1;
        if (a instanceof CommunityInfo && app.communities.isStarred(null, a.id))
          return -1;
        return 0;
      })
      .filter((item) => {
    ...
    const currentCommunity = allCommunities.find((item) => {
      ...
      if (item instanceof CommunityInfo)
        return app.activeCommunityId() === item.id;
      return false;
    });
    ...
    const isInCommunity = (item) => {
      if (item instanceof ChainInfo) {
        ...
        return (
          app.user.getAllRolesInCommunity({ community: item.id }).length > 0
        );
      } else {
        return false;
      }
    ...
    return m('.CommunitySelector', [
      m('.title-selector', [
        m(PopoverMenu, {
          ...
          trigger: m(Button, {
            rounded: true,
            label: vnode.attrs.showTextLabel
              ? activeEntityName
