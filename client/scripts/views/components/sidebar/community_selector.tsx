  ...
  implements m.ClassComponent<CommunitySelectorAttrs>
{
  view(vnode) {
    ...
    const starredCommunities = allCommunities.filter((a) => {
      return a instanceof ChainInfo && app.communities.isStarred(a.id, null);
    });
    const joinedCommunities = allCommunities.filter(
      (c) => isInCommunity(c) && !app.communities.isStarred(a.id, null)
    );
