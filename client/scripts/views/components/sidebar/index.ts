...
const SidebarQuickSwitcher: m.Component<{}> = {
  view: (vnode) => {
    ...
    const starredCommunities = allCommunities.filter((item) => {
      ...
      if (item instanceof ChainInfo && !app.communities.isStarred(item.id, null)) {
        return false;
      }
      return true;
    });
      ...
      discord &&
        m(Tooltip, {
          ...
          trigger: m(Button, {
            ...
            label: m(CWIcon, { iconName: 'discord' }),
...
const Sidebar: m.Component<
  { hideQuickSwitcher?; useQuickSwitcher?: boolean },
  {}
> = {
  view: (vnode) => {
    ...
    return [
      ...
      !useQuickSwitcher &&
        ...
        m(`.Sidebar${isCustom ? '.custom-domain' : ''}`, [
          ...
          app.isLoggedIn() &&
            app.chain &&
            m('.subscription-button', m(SubscriptionButton)),
