...
const CHAINBASE_WITH_CLI = [ChainBase.CosmosSDK, ChainBase.Substrate];
...
const LoginWithWalletDropdown: m.Component<{
  ...
  view: (vnode) => {
    ...
    const web3loginParams = loggingInWithAddress
      ? { prev, loggingInWithAddress, targetCommunity }
      : joiningChain
      ? { prev, joiningChain }
      : joiningCommunity
      ? { prev, joiningCommunity }
      : { prev };
    ...
    const sortedChainBasesWithCLI = sortedChainBases.filter(
      (base) => CHAINBASE_WITH_CLI.indexOf(base) !== -1
    );

    const getMenuItemForChainBase = (base: ChainBase, cli?: boolean) =>
      m(MenuItem, {
    ...
    const menuItems =
      chainbase && CHAINBASE_WITH_CLI.indexOf(chainbase) !== -1
        ? [
        ...
        ? [getMenuItemForChainBase(chainbase)]
        : sortedChainBases
            .map((base) => getMenuItemForChainBase(base))
            .concat(sortedChainBasesWithCLI.length > 0 ? m(MenuDivider) : null)
            .concat(
              sortedChainBasesWithCLI.map((base) =>
                getMenuItemForChainBase(base, true)
              )
            );
