  ...
  view: (vnode) => {
    ...
    return m(
      ...
      [
              ...
              m(Button, {
                ...
                onclick: async (e) => {
                  ...
                  try {
                    ...
                    if (
                      legacyResponse.status === 'Success' &&
                      legacyResponse.result?.shouldUseMagic
                    ) {
                      ...
                      setTimeout(() => {
                        ...
                        vnode.state.showMagicLoginPromptEmail = $(e.target)
                          .closest('.Login')
                          .find('[name="email"]')
                          .val()
                          .toString();
                ...
                label: 'Go',
                loading:
                  vnode.state.disabled && !vnode.state.showMagicLoginPrompt,
                disabled:
                  vnode.state.disabled && !!vnode.state.showMagicLoginPrompt,
              }),
            ]),
        ...
        vnode.state.failure
          ? m('.login-message.failure', [
              vnode.state.error || 'An error occurred.',
            ])
          : vnode.state.emailLoginSucceeded
          ? m('.login-message.success', ['Check your email to continue.'])
          : vnode.state.showMagicLoginPrompt
          ? m('.login-magic-prompt', [
              m('p', ['Do you have a crypto wallet already?']),
                  ...
                  onclick: async (e) => {
                    ...
                    try {
                      ...
                      await loginWithMagicLink(
                        vnode.state.showMagicLoginPromptEmail
                      );
                      vnode.state.disabled = false;
                      // do not redirect -- just close modal
                      exitWithMagicLoginComplete();
