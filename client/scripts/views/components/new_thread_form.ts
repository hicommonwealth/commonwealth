...
export const loadDraft = async (dom, state, draft) => {
  ...
  localStorage.setItem(
    `${app.activeId()}-new-discussion-storedTitle`,
  ...
  oninit: (vnode) => {
    ...
    if (vnode.state.postType === PostType.Discussion) {
      ...
      vnode.state.form.url = localStorage.getItem(
        `${app.activeId()}-new-link-storedLink`
      );
      vnode.state.form.linkTitle = localStorage.getItem(
        `${app.activeId()}-new-link-storedTitle`
      );
    }
  },
  ...
  view: (vnode) => {
    ...
    const clearLocalStorage = (type: PostType) => {
      if (type === PostType.Discussion) {
        ...
        localStorage.getItem(`${app.activeId()}-post-type`) === PostType.Link
      ) {
          ...
          m(FormGroup, [
            m(
              ...
              [
                ...
                isModal &&
                  m.route.get() !== `${app.activeId()}/new/thread` &&
                  m(TabItem, {
                    class: 'tab-right',
                    label: [
                      'Full editor',
              ...
              m(FormGroup, { order: 5 }, [
                ...
                m(Button, {
                  ...
                  onclick: async (e) => {
                    ...
                    const fromDraft_ = vnode.state.recentlyDeletedDrafts.includes(
                      vnode.state.fromDraft
                    )
                      ? undefined
                      : vnode.state.fromDraft;
