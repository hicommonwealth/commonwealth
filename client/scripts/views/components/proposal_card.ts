...
export const getStatusText = (
  ...
  return proposal instanceof MolochProposal &&
    proposal.state === MolochProposalState.NotStarted
    ? 'Waiting to start'
    : proposal instanceof MolochProposal &&
      proposal.state === MolochProposalState.GracePeriod
    ? [
    ...
    : proposal instanceof MolochProposal &&
      proposal.state === MolochProposalState.InProcessingQueue
    ? 'In processing queue'
    : proposal instanceof MolochProposal &&
    ...
    : proposal.isPassing === ProposalStatus.Passing
    ? ['Expected to pass, ', countdown]
    : proposal.isPassing === ProposalStatus.Failing
    ? ['Needs more votes, ', countdown]
    ...
    return m('.ProposalCard', [
      m(
        ...
        [
          ...
          proposal instanceof SubstrateTreasuryProposal &&
            m('.proposal-amount', proposal.value.format(true)),
          proposal instanceof SubstrateDemocracyReferendum &&
            m('.proposal-amount', proposal.threshold),
          ...
          proposal instanceof SubstrateDemocracyReferendum &&
            ...
            (() => {
              ...
              if (
                ...
                return m('.proposal-action', [
                  'Via PROP-',
                  originatingProposalOrMotion.identifier,
                ]);
              } else if (
                originatingProposalOrMotion instanceof
      ...
      m(
        '.proposal-card-bottom',
        {
          onclick: (e) => {
            ...
            if (proposal?.threadId) {
              m.route.set(
                `/${app.activeId()}/proposal/discussion/${proposal.threadId}`
              );
            }
          },
        },
        [
          // thread link
          proposal.threadId
            ? m('.proposal-thread-link', [
                m(
                  'a',
                  {
                    href: `/${app.activeId()}/proposal/discussion/${
                      proposal.threadId
                    }`,
                  },
                  'Go to thread'
