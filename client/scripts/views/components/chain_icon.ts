...
export const ChainIcon: m.Component<{
  chain: ChainInfo;
  onclick?: Function;
  size?: number;
}> = {
...
export const ChainBaseIcon: m.Component<{
  chainbase: ChainBase;
  onclick?: Function;
  size?: number;
}> = {
  view: (vnode) => {
    ...
    const size = vnode.attrs.size || 32;
    const iconName =
      vnode.attrs.chainbase === ChainBase.Ethereum
        ? 'eth'
        : vnode.attrs.chainbase;
...
export const CommunityIcon: m.Component<{
