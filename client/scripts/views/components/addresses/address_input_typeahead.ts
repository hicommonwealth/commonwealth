...
const AddressInputTypeahead: m.Component<
  { options: any; oninput },
  { selectedItem; typeaheadAddresses; loading; initialized }
> = {
  oncreate: (vnode) => {
      ...
      itemRender: (item: AddressInputTypeaheadItem) =>
        m(ListItem, {
          label: [
            ...
            item.name
              ? [m('strong', item.name), ' ', m('span.lighter', item.address)]
              : item.address,
          ],
          selected:
            vnode.state.selectedItem &&
            vnode.state.selectedItem.address === item.address,
        }),
