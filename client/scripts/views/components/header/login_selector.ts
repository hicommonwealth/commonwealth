...
> = {
  view: (vnode) => {
    ...
    const samebaseAddresses = app.user.addresses.filter((addr) =>
      joiningChain
        ? networkToBase(addr.chain) === networkToBase(joiningChain)
        : true
    );

    const samebaseAddressesFiltered = samebaseAddresses.reduce(
      (arr, current) => {
        if (
          !arr.find(
            (item) =>
                ...
                try {
                  ...
                  if (res && res.result) {
                    ...
                    const addressInfo = app.user.addresses.find(
                      (a) => a.address === address && a.chain === targetChain
                    );
                ...
                m(MenuItem, {
                  onclick: () => {
                    ...
                    if (app.chain) {
                      ...
                      m.route.set(
                        `/${app.activeId()}/account/${pf.address}?base=${
                          pf.chain || a.chain.id
                        }`
                      );
                    }
