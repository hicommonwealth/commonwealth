...
const getBalanceTransferChecks = (
  ...
  if (senderAddress === recipientAddress) {
    ...
    senderBalance
      .sub(amount)
      .lt((app.chain as Substrate).chain.existentialdeposit)
  ) {
    checks.push([
  ...
  const resultingBalance = app.chain.chain.coins(
    recipientBalance.add(amount.sub(txFee))
  );
  if (
    recipientBalance.eqn(0) &&
    resultingBalance.lt((app.chain as Substrate).chain.existentialdeposit)
  ) {
    checks.push([
      m(Icon, { name: Icons.X, size: 'sm' }),
    ...
    checks.push([
      m(Icon, { name: Icons.INFO, size: 'sm' }),
      `Recipient's balance after this transfer: ${formatCoin(
        resultingBalance
      )}`,
    ]);
  }
  ...
  oninit: (vnode) => {
    app.runWhenReady(async () => {
      ...
      vnode.state.substrateTxFee =
        vnode.attrs.sender.chainBase === ChainBase.Substrate
  ...
  view: (vnode: m.VnodeDOM<IAttrs, IState>) => {
    ...
    return m('.SendEDGWell', [
      m(
        'h4',
        `Send ${
          (app.chain && app.chain.chain && app.chain.chain.denom) || 'balance'
        }`
      ),
      m('p', [
        m(AddressInput, {
          currentPrefix:
            app.chain.base === ChainBase.Substrate
              ? (app.chain as Substrate).chain.ss58Format
              : null,
          autoReencode: true,
          options: {
            name: 'address',
        ...
        m('input[type="text"]', {
          ...
          oninput: (e) => {
            try {
              vnode.state.amount = app.chain.chain.coins(
          ...
          onclick: async (e) => {
            ...
            try {
              ...
              if (sender instanceof SubstrateAccount) {
                ...
                throw new Error(
                  'Can only send balance on Substrate-based networks'
                );
              }
            } catch (err) {
