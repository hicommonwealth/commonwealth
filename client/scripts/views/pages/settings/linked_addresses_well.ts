...
const AccountRow: m.Component<
  { account: AddressInfo; onclick?: (e: Event) => any },
  { removing }
> = {
  view: (vnode): m.Vnode => {
      ...
      [
        ...
        m('.action-col', [
          m(Button, {
            ...
            rounded: true,
            disabled:
              vnode.state.removing || app.user.addresses.some((a) => a.isMagic),
...
const LinkedAddressesWell: m.Component<{}> = {
  view: () => {
    ...
    return m('.LinkedAddressesWell', [
      ...
      app.user.addresses.length === 0 && m('.no-accounts', 'No addresses'),
    ]);
  },
};
