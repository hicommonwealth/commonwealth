...
const Chat: m.Component<IAttrs, IState> = {
  oninit: (vnode) => {
    ...
    const onIncomingMessage = (text, author, author_chain, timestamp?) => {
      // timestamp is used for scrollback only
    ...
    const server = CHAT_SERVER.startsWith('localhost')
      ? `ws://${CHAT_SERVER}`
      : `wss://${CHAT_SERVER}`;
    const chatRoomUrl = `${server}/?room=${vnode.attrs.room}`;
    vnode.state.chat = new ChatController(
      chatRoomUrl,
      app.user.jwt,
      (connected) => {
        if (connected) {
    ...
    vnode.state.chat.addListener(
      WebsocketMessageType.Message,
      onIncomingMessage
    );
    vnode.state.chat.addListener(
      WebsocketMessageType.Typing,
      onIncomingTypingIndicator
    );
    vnode.state.chat.addListener(
      WebsocketMessageType.InitializeScrollback,
      onIncomingMessage
    );
    vnode.state.messages = [];
    vnode.state.outgoingTypingInputHandler = _.throttle(
      (e) => {
        if (!vnode.state.chat.isConnected) return;
        if (e && e.target && $(e.target).val() === '') return;
        vnode.state.chat.sendTypingIndicator(app.user.jwt);
      },
      TYPING_INDICATOR_OUTGOING_FREQUENCY,
  ...
  view: (vnode) => {
    ...
    return m('.Chat', [
        ...
        groupedMessages.map((grp) =>
          m('.chat-message-group', [
            ...
            m(
              ...
              formatTimestampForChat(grp.messages[0].timestamp)
            ),
            m('.clear'),
      ...
      !app.isLoggedIn()
        ? m('.chat-composer-unavailable', 'Log in to join chat')
        : !app.user.activeAccount
        ? m('.chat-composer-unavailable', 'Set up account to join chat')
        : !vnode.state.chat.isConnected
        ? m('.chat-composer-unavailable', 'Waiting for connection')
        : m('form.chat-composer', [
            m(ResizableTextarea, {
              ...
              placeholder: vnode.state.chat.isConnected
                ? 'Enter a message...'
                : 'Disconnected',
