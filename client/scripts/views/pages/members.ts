...
const MembersPage: m.Component<
  {},
  { membersRequested: boolean; membersLoaded: MemberInfo[] }
> = {
  view: (vnode) => {
      ...
      activeInfo.getMembers(activeInfo.id).then(() => {
        ...
        if (
          ...
          setTimeout(() => {
            window.scrollTo(
              0,
              Number(localStorage[`${app.activeId()}-members-scrollY`])
            );
          }, 100);
        }
      });
              ...
              'a.members-item',
              {
                ...
                onclick: (e) => {
                  ...
                  localStorage[`${app.activeId()}-members-scrollY`] =
                    window.scrollY;
                  m.route.set(
                    `/${app.activeId()}/account/${info.address}?base=${
                      info.chain
                    }`
                  );
                },
