...
const search = _.debounce((searchTerm, vnode) => {
  ...
  $.get(`${app.serverUrl()}/search`, params)
    .then((response) => {
...
const SearchPage: m.Component<
  {},
  { results; searchLoading; searchTerm; errorText: string }
> = {
  view: (vnode) => {
      ...
      [
        m(Input, {
          ...
          oninput: (e) => {
            ...
            m.route.set(
              `/${app.activeId()}/search?q=${encodeURIComponent(
                searchTerm.toString().trim()
              )}`
            );
          },
        }),
        vnode.state.searchLoading
          ? m('.search-loading', [
              m(Spinner, {
                active: true,
                fill: true,
                ...
                m(
                  'a.search-results-clear',
                  {
                    ...
                    onclick: (e) => {
                      ...
                      $('.search-page-input input').val('').select().focus();
                    },
                  },
                  'Clear'
                ),
                              ...
                              (() => {
                                try {
                                  const doc = JSON.parse(
                                    decodeURIComponent(result.body)
                                  );
