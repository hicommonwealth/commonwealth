...
const clearEditingLocalStorage = (item, isThread: boolean) => {
  if (isThread) {
    localStorage.removeItem(
      `${app.activeId()}-edit-thread-${item.id}-storedText`
    );
  } else {
    localStorage.removeItem(
      `${app.activeId()}-edit-comment-${item.id}-storedText`
    );
  }
};
      ...
      onclick: async (e) => {
        ...
        if (
          ...
          const confirmed = await confirmationModalWithText(
            'Unsent comments will be lost. Continue?'
          )();
          if (!confirmed) return;
        }
        ...
        Button,
        {
          ...
          onclick: (e) => {
            ...
            if (item instanceof OffchainThread) {
              app.threads
                .edit(item, itemText, parentState.updatedTitle)
                .then(() => {
                  m.route.set(
                    `/${app.activeId()}/proposal/${item.slug}/${item.id}`
                  );
                  parentState.editing = false;
