...
> = {
  view: (vnode) => {
    ...
    const commentLink =
      `/${app.activeId()}/proposal/${proposal.slug}/` +
...
> = {
  view: (vnode) => {
      ...
      return comment.childComments.map((id) => {
        ...
        return m('.threading-level', [
          ...
          !!child.childComments.length &&
            m(
              '.child-comments-wrap',
              recursivelyGatherChildComments(child, replyParent2)
            ),
        ]);
      });
    };
      ...
      return comments2.map((comment, index) => {
        return [
          ...
          !!comment.childComments.length &&
            m(
              '.child-comments-wrap',
              recursivelyGatherChildComments(comment, replyParent2)
            ),
          replyParent2 &&
    ...
    return m(
      ...
      [
        // show comments
        comments && m('.proposal-comments', AllComments(comments, replyParent)),
        // create comment
        app.user.activeAccount &&
...
> = {
  ...
  view: (vnode) => {
    ...
    if (identifier !== `${proposalId}-${slugify(proposal.title)}`) {
      m.route.set(
        `/${app.activeId()}/proposal/${proposal.slug}/${proposalId}-${slugify(
          proposal.title
        )}`,
        {},
        { replace: true }
    ...
    const getSetGlobalReplyStatus = (
      call: string,
      parentId?: number | boolean,
      suppressScrollToForm?: boolean
    ) => {
        ...
        setTimeout(() => {
          ...
          if (scrollTop + $(window).height() < replyTop + $reply.outerHeight())
            $('html, body').animate(
              {
                scrollTop:
                  replyTop + $reply.outerHeight() - $(window).height() + 40,
              },
              500
            );
