...
const ChainCard: m.Component<{ chain: string; nodeList: NodeInfo[] }> = {
  view: (vnode) => {
    ...
    return m(
      ...
      [
        m('.card-left', [m(ChainIcon, { chain: nodeList[0].chain })]),
        m('.card-right', [
          m('.card-right-top', [m('h3', chainInfo.name)]),
            ...
            !!monthlyThreadCount && [
              m(
                ...
                monthlyThreadCount > 20
                  ...
                  : [pluralize(monthlyThreadCount, 'thread'), ' / month']
              ),
              app.user.isMember({
...
const CommunityCard: m.Component<{ community: CommunityInfo }> = {
  view: (vnode) => {
    ...
    return m(
      ...
      [
        m('.card-left', [m(CommunityIcon, { community })]),
        m('.card-right', [
          m('.card-right-top', [
            m('h3', [
              community.name,
