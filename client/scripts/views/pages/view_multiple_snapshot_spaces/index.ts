...
function redirectHandler(action: string, proposal: null | OffchainThread): {header_message: string, redirect_option: string, proposal: null | OffchainThread} {
...
const MultipleSnapshotsPage: m.Component<{
    ...
    spaces_metadata: Array<{space: SnapshotSpace, proposals: SnapshotProposal[]}>
}> = {
    view: (vnode) => {
        ...
        if (app.chain && !vnode.state.snapshot_spaces) {
            vnode.state.snapshot_spaces = app.config.chains!.getById(app.activeChainId()).snapshot;
            m.redraw();
        }
