...
> = {
  view: (vnode) => {
    ...
    const { attachments, author, body, title, createdAt, chain, community } =
      proposal;

    // hide rows from communities that don't match
    if (app.isCustomDomain() && (chain || community) !== app.customDomainId())
      return;
        ...
        m('', [
          proposal.kind === OffchainThreadKind.Question
            ...
            : [
                ...
                link(
                  'a.link-bold',
                  `/${chain || community}${getProposalUrlPath(
                    slug,
