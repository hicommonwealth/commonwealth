...
const ProjectCard: m.Component<ProjectCardAttrs, ProjectCardState> = {
  view: (vnode) => {
    ...
    const ProjectCardLarge = m('.ProjectCard', { class: 'large', onclick }, [
    ...
    const ProjectCardMedium = m('.ProjectCard', { class: 'medium', onclick }, [
    ...
    const ProjectCardSmall = m('.ProjectCard', { class: 'small', onclick }, [
      m('.top-panel', [
        m('h3', project.title),
        // TODO: Implement label in kit
        m(`.project-status.${projectStatus}`, capitalize(projectStatus)),
      ]),
      m('.bottom-panel', [
        m(DummyChainIcon, {
          chain: null,
