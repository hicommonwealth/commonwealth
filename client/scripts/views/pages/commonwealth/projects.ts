      ...
      PROJECTS.map((project) => {
        return m(Card, {
          ...
          m('.project-metadata', [
            ...
            m('.project-metadata-expires', [
              'Expires in ',
              moment(Date.now()).diff(project.expires_at, 'days'),
              ' days',
            ]),
            m('.project-metadata-beneficiary', [
              m(User, { user: new AddressInfo(null, project.beneficiary_address, project.beneficiary_chain, null) }),
            ]),
          ]),
          // backing progress
          m('.project-metrics', [
            m('.project-metric', [
              ...
              m('.project-metric-title', 'Curating this project'),
            ]),
            m('.project-metric', [
              m('.project-metric-figure', `${project.backer_amount.toFixed(1)} ETH`),
              m('.project-metric-title', 'Backing this project'),
            ]),
          ]),
          m('.project-progress', [
            m('.project-progress-bar', [
              m('.project-progress-fill', {
                style: `width: ${(100 * (project.backer_amount / project.backer_required)).toFixed(1)}%`
              }),
            ]),
            m('.project-progress-text', [
              `${(100 * (project.backer_amount / project.backer_required)).toFixed(1)}% backed`
            ]),
          ]),
        ]);
      })
