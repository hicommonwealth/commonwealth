...
const DiscussionFilterBar: m.Component<{ topic: string; stageId: number, parentState; disabled: boolean; }, {}> = {
  view: (vnode) => {
    const { topic, stageId, disabled } = vnode.attrs;

    const communityInfo = app.chain?.meta?.chain || app.community?.meta;
    if (!communityInfo) return;
    const { stagesEnabled } = communityInfo;
      ...
      stagesEnabled &&
        m(PopoverMenu, {
          trigger: m(Button, {
            ...
            class: 'stage-filter',
            label: selectedStage
              ? `Stage: ${offchainThreadStageToLabel(selectedStage.name)}`
              : 'All Stages',
            iconRight: Icons.CHEVRON_DOWN,
            size: 'sm',
            disabled,
