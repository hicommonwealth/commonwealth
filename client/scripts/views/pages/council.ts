...
const Councillor: m.Component<{ account }> = {
  view: (vnode) => {
    ...
    const hasMyVote =
      ...
      votes.filter((v) => v.account === app.user.activeAccount);
...
const CouncilElectionVoter: m.Component<{
  ...
  view: (vnode) => {
    ...
    const canBeReaped: boolean = (app.chain as Substrate).phragmenElections.activeElection.isDefunctVoter(
      voterAccount
    );
    const isPresentationPhase = false;
    const lastActive: number = null;
    const stake: string = voter.stake.format();
    const canBeRetracted =
      app.user.activeAccount && voterAccount.address === myAccount.address;

    return link(
      'a.CouncilElectionVoter',
      `/${voterAccount.chain.id}/account/${voterAccount.address}`,
      [
        m('.col-member', m(UserBlock, { user: voterAccount })),
        m('.col-info', [
          m('.metadata-item', `Locked ${stake}`),
          (canBeReaped || canBeRetracted) &&
            m(
              'a',
              {
                href: '#',
                class:
                  isPresentationPhase || !app.user.activeAccount
                    ? 'disabled'
                    : '',
                onclick: (e) => {
                  e.preventDefault();
                  if (canBeRetracted) {
                    ...
                    createTXModal(
                      (app.chain as Substrate).phragmenElections.activeElection.reportDefunctVoterTx(
                        myAccount,
                        voterAccount
                      )
                    );
              ...
              [
                isPresentationPhase
                  ? 'Can only claim in voting phase'
                  : canBeRetracted
                  ? 'Retract vote'
...
const CouncilPage: m.Component<{}> = {
  ...
  view: (vnode) => {
    ...
    const nRunnersUpSeats = (app.chain as Substrate).phragmenElections
      .desiredRunnersUp;
    const termDuration = (app.chain as Substrate).phragmenElections
      .termDuration;
    const votingBond = formatCoin(
      (app.chain as Substrate).phragmenElections.votingBond
    );
    const nextRoundStartBlock = (app.chain as Substrate).phragmenElections
      .activeElection.endTime.blocknum;
    const candidacyBond = formatCoin(
                  ...
                  m(
                    'a',
                    {
                      onclick: (e) => {
                        ...
                        m.route.set(
                          `/${app.activeChainId()}/new/proposal/:type`,
