...
const CouncilElectionVoter: m.Component<{
  ...
  view: (vnode) => {
    ...
    const canBeReaped: boolean = (
      app.chain as Substrate
    ).phragmenElections.activeElection.isDefunctVoter(voterAccount);
                ...
                onclick: (e) => {
                  ...
                  if (canBeRetracted) {
                    createTXModal(
                      (
                        app.chain as Substrate
                      ).phragmenElections.activeElection.removeVoterTx(
                        myAccount
                      )
                    );
                  } else if (canBeReaped) {
                    createTXModal(
                      (
                        app.chain as Substrate
                      ).phragmenElections.activeElection.reportDefunctVoterTx(
                        myAccount,
                        voterAccount
                      )
                    );
...
const CouncilPage: m.Component<{}> = {
  ...
  view: (vnode) => {
    ...
    const voters = (
      app.chain as Substrate
    ).phragmenElections.activeElection?.getVoters();
