...
const ValidatorsPage: m.Component<
  {},
  { validators; totalStaked; validatorsInitialized: boolean }
> = {
  view: (vnode) => {
    ...
    if (
      ...
      !vnode.state.validatorsInitialized
    ) {
      (app.chain as Substrate).accounts.getValidators().then((result) => {
        ...
        (result as any).forEach((va) => {
          vnode.state.totalStaked = (app.chain as Substrate).chain.coins(
            vnode.state.totalStaked.asBN.add(va.total.asBN)
          );
        });
      });
    ...
    return m(
      ...
      [
        ...
        (sort === 'amount'
          ? validators?.sort(
              (a, b) => b.total?.toString() - a.total?.toString()
            )
          : sort === 'nominators'
