...
const ProposalsPage: m.Component<{}> = {
  ...
  view: (vnode) => {
    ...
    const activeMarlinProposals =
      onMarlin &&
      (app.chain as Marlin).governance.store
        .getAll()
        .filter((p) => !p.completed)
    ...
    const activeProposalContent =
      ...
      !activeMarlinProposals?.length
        ...
        : (activeDemocracyProposals || [])
            ...
            .concat(
              (activeMarlinProposals || []).map((proposal) =>
                m(ProposalCard, { proposal })
              )
            );
    ...
    const inactiveProposalContent =
      ...
      !inactiveMarlinProposals?.length
        ...
        : (inactiveDemocracyProposals || [])
            ...
            .concat(
              (inactiveMarlinProposals || []).map((proposal) =>
                m(ProposalCard, { proposal })
              )
            );
