...
const ChainManager: m.Component<IChainManagerAttrs, IChainManagerState> = {
  view: (vnode) => {
    const nodeRows = (chain: ChainInfo) =>
      (app.config.nodes.getByChain(chain.id) || []).map((node, nodeIndex) => {
    ...
    return m('.ChainManager', [
      ...
      (app.config.communities.getAll() || []).map((community) =>
        m('.chain-row', [
          m('h3', [m('strong', community.name)]),
          m(
            '.chain-subtitle',
            {
              style: 'margin: -14px 0 10px; color: #999;',
