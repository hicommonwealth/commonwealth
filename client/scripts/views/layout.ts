...
> = {
  view: (vnode) => {
    ...
    const scopeMatchesCommunity = app.config.communities
      .getAll()
      .find((c) => c.id === scope);
    ...
    if (app.loadingError) {
      ...
      return m(
        ...
        [m(PageNotFound), m(AppModals), m(AppToasts)]
      );
    } else if (
      scope &&
      scope !== app.activeId() &&
    ...
    return m(
      '.Layout.mithril-app',
      {
        class: `${hideSidebar ? 'hide-sidebar' : ''} ${
          app.isCustomDomain() ? 'custom-domain' : ''
        }`,
      },
      [vnode.children, m(AppModals), m(AppToasts)]
    );
  },
};
