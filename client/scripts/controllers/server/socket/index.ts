...
class WebsocketController {
  ...
  public _init: () => void;
  private _listeners: {
    [key in WebsocketMessageType]: WebsocketMessageHandler;
  };

  constructor(url, jwt, onStatusChange) {
    this._listeners = Object.assign(
      {},
      ...Object.values(WebsocketMessageType).map((key) => ({
    ...
    this._init = () => {
      ...
      const heartbeatTimer = setInterval(() => {
        const heartbeat: IWebsocketsPayload<any> = {
          event: WebsocketMessageType.Heartbeat,
          jwt,
        };
  ...
  public addListener(
    ...
    listener: WebsocketMessageHandler
  ) {
