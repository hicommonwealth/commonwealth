...
> {
  ...
  public get candidates() {
    return [
      ...
      ...this._Elections.members,
      ...this._Elections.runnersUp,
    ];
  }
  ...
  public updateVoters = async () => {
    ...
    const candidates = (await this._Chain.api.query[
      this.moduleName
    ].candidates()) as Vec<AccountId>;
    ...
    if (this._Chain.api.query[this.moduleName].voting) {
      const voting = await this._Chain.api.query[
        this.moduleName
      ].voting.entries();
      for (const [key, [stake, votes]] of voting as Array<
        [StorageKey, [BalanceOf, Vec<AccountId>] & Codec]
      >) {
      ...
      const [voters, votes] = voting as [Vec<AccountId>, Vec<Vec<AccountId>>] &
        Codec;
      const stakes = (await this._Chain.api.queryMulti(
        voters.map((who) => [
          this._Chain.api.query[this.moduleName].stakeOf,
          who,
        ])
      )) as BalanceOf[];

      const voteDataArray = _.zip(voters, votes, stakes) as Array<
        [AccountId, Vec<AccountId>, BalanceOf]
      >;
  ...
  public reportDefunctVoterTx(
    ...
    return this._Chain.createTXModalData(
      reporter,
      (api: ApiPromise) =>
        api.tx[this.moduleName].reportDefunctVoter(voter.address),
      'reportDefunctVoter',
      this.title,
      () => this.updateVoters()
    );
