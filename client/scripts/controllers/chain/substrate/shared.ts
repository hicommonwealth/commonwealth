...
class SubstrateChain implements IChainModule<SubstrateCoin, SubstrateAccount> {
  ...
  public get reservationFee() {
    return this._reservationFee;
  }
  ...
  public async initMetadata(): Promise<void> {
    ...
    const reservationFee = this.api.consts.nicks
      ? (this.api.consts.nicks.reservationFee as Balance)
      : null;
    ...
    const blockNumbers = [...Array(TIMESTAMP_LOOKBACK).keys()].map(
      (i) => +blockNumber - i - 1
    );
    const hashes = await this.api.query.system.blockHash.multi<Hash>(
      blockNumbers
    );
    const timestamps = await Promise.all(
      hashes.map((hash) => this.api.query.timestamp.now.at(hash))
    );
              ...
              for (const e of result.events) {
                if (this.api.events.system.ExtrinsicSuccess.is(e.event)) {
                  ...
                  if (errorData.isModule) {
                    const details = this.registry.findMetaError(
  ...
  public methodToTitle(method): string {
    ...
    const args = (method.args as any[]).map((arg, index) => {
      ...
      switch (argType) {
        ...
        case 'Compact<Moment>':
          return moment(
            new Date(this.createType('Compact<Moment>', arg).toNumber())
          )
            .utc()
            .toString();
        case 'Compact<Balance>':
          return formatCoin(
            this.coins(this.createType('Compact<Balance>', arg))
          );
        default:
