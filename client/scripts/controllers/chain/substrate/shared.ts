...
class SubstrateChain implements IChainModule<SubstrateCoin, SubstrateAccount> {
  ...
  public initChainEntities(): Promise<void> {
    this._fetcher = new SubstrateEvents.StorageFetcher(this._apiPromise as any); // Todo: REMOVE "as any" when chainevents get removed
    const subscriber = new SubstrateEvents.Subscriber(this._apiPromise as any); // Todo: REMOVE "as any" when chainevents get removed
    const processor = new SubstrateEvents.Processor(this._apiPromise as any); // Todo: REMOVE "as any" when chainevents get removed
