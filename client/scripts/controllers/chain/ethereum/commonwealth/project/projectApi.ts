...
export default class CMNProjectApi extends ContractApi<CMNProjectContract> {
    ...
    try {
      ...
      const tx =
        token === EtherAddress
          ...
          : await contract.back(token, amount.toString(), {
    ...
    try {
      ...
      const tx =
        token === EtherAddress
          ...
          : await contract.curate(token, amount.toString(), {
              gasLimit: this.gasLimit,
            });
      const txReceipt = await tx.wait();
      return txReceipt.status === 1;
  ...
  public async redeemTokens(
    ...
    try {
      ...
      const tx = isBToken
        ? await contract.redeemBToken(token, amount.toString(), {
            gasLimit: this.gasLimit,
          })
        : await contract.redeemCToken(token, amount.toString(), {
            gasLimit: this.gasLimit,
          });
      const txReceipt = await tx.wait();
      return txReceipt.status === 1;
  ...
  public async setProjectDetails(project: CMNProject) {
    ...
    const acceptedTokenAddresses = acceptedTokens.map((t) =>
      t.address.toLowerCase()
    );
    const bTokens = {};
    const cTokens = {};
    for (let i = 0; i < acceptedTokenAddresses.length; i++) {
      bTokens[acceptedTokenAddresses[i]] = (
        await this.Contract.getBToken(acceptedTokenAddresses[i])
      ).toLowerCase();
      cTokens[acceptedTokenAddresses[i]] = (
        await this.Contract.getCToken(acceptedTokenAddresses[i])
      ).toLowerCase();
    }
  ...
  public async getProjectInfo() {
    ...
    const funded = await this.Contract.funded();
    const endTimeBN = (await this.Contract.deadline()).mul(
      BigNumber.from(1000)
    );
