...
import EvmKeplrWalletController from './webWallets/evm_keplr_web_wallet';
...
export default class WebWalletController {
  ...
  public availableWallets(chain?: ChainBase): IWebWallet<any>[] {
    ...
    if (app.chain?.meta?.id) {
      const specificWallet = this._wallets.filter((w) => w.specificChains?.includes(specificChain));
