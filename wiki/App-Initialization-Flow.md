...
2. `index.tsx` uses the document’s `root` element as a container to render the `App` view component(imported from `app.tsx`) within.
3. The `App` component fires the `useInitApp()` hook (`hooks/useInitApp.tsx`)
    1. `useInitApp()` sets the `isLoading` and `customDomain` state variables, as well as creating setter functions `setCustomDomain` and `setIsLoading`.
    2. `useInitApp()` queries `/domain`, setting the `customDomain` state variable with the query's response.
    3. The `App` component defines local variables `customDomain` and `isLoading` from the return values of `useInitApp()`.
4. `useInitApp()` passes the `customDomain` variable to `initAppState()` (`state/index.ts`), which hits our `/status` endpoint.
    1. The `app` state variable is defined within `state/index.ts`, and its values are updated by the `initAppState`, using the `data` object returned by the `/status` route.
        - `initAppState()` is called on: app initialization, user login, user logout, and chain community creation.
...
5. The `/status` endpoint is divided into two main data-fetches, `getChainStatus()` and `getUserStatus()`. `getChainStatus()` is always called; `getUserStatus()` is only called if a `user` object has been passed with the request.
    1. `getChainStatus()`: grabs ChainStore, ChainNodes, NotificationCategories, and CommunitySnapshotSpaces, and threadCountQueryData
    2. `getUserStatus`: User associations (e.g. selected chain instance, social accounts, addresses) fetched. Once addresses have been grabbed, various rows associated with those addresses (e.g. discussion drafts, roles) follow.
...
9. If `selectChain()` (`/helpers/chain.ts`) is fired, per step #8:
    1. If no `chain` argument is passed, the function defaults to a `chain` value set my `app.user.selectedChain`, or else `app.config.defaultChain`.
        - _Note: This is very likely dead code that may be removed._
    ...
    6. Dark mode preferences are set, forum data (threads, admins, banners, recent activity) are set. The `app.chain` state variable is set globally to the now-server-initialized `IChainAdapter` instance.
    7. If the `initChain` argument is `true` (e.g. in the case of NEAR communities), we then proceed to `initChain()`.
        ...
        3. If the chain's data is not loaded, `app.chain.initData()` is fired.
            - This call triggers chain-specific setup of governance data in particular. It will (generally, as part of chain-specific setup) also call `activeAddressHasToken` which hits `/tokenBalance` for the logged in user. It also emits `ready` from `chainModuleReady` event emitter.
        4. Emit `ready` from the `chainAdapterReady` event emitter, and print `CHAIN started.` from the console.
    8. If the user is logged in, his addresses for the initialized community areset, and the `/selectChain` endpoint is hit, to update his last visited community.
10. The `Sublayout` component renders the header, sidebar, banners, and similar sub-components, as well as wrapping many content pages. 